...
- Builtin presets: `autoresearch`, `ce-executor-pipeline` (14-hat 线性一条龙 **Ralph primary path**: plan-reviewer → executor(每个 U-ID 一个 subagent,主 executor 负责验收/提交/最终 emit) → `test-stabilizer`(2026-07-16-001 U3,executor 后 test 稳定化门禁,可改测试+生产代码,带 correction ID 与全量门禁) → 6 串行维度 hat → review-synthesizer → fix-planner → fixer → alignment → reporter), `ce-executor-pipeline-loop` (`ce-executor-pipeline` 的环形 review/fix 版本: work.done/fix.done → `test-stabilizer`(每轮修复后再次稳定化,U5) → review-reentry → 6 串行维度 review → review-gate；按 `must_fix_now` 收敛门控继续修复,baseline / 范围外 / 新发现残留进入报告,最多 6 轮), `ce-executor-supervisor` (2026-07-03-001 plan: 16-hat; rusqlite-backed supervisor wave orchestration, per-slot worktrees, fan-in merge, parallel review/fix; 需用 `--features supervisor-db` 编译且 `event_loop.supervisor.enabled: true`), `ce-executor-lite` (template), `debug`, `merge-batch` (Git-first 多 worktree 批量 merge: reviewer → integrator → stabilizer 自环 → reporter), `merge-loop`(内部单 loop 自动 merge;裸 `ce-executor` 已删除:所有 plan-driven 执行请使用 `ce-executor-pipeline`;仅作模板时可使用 `ce-executor-lite`)
- `presets/index.json` is the user-facing preset manifest
- 线性 `ce-executor-pipeline` 的 `fix.done` 必须再次经过 `test-stabilizer` 与六维独立复审；`review_phase: post_fix` 只能接受或阻塞，禁止直通 Alignment 或开启第二份 Fix Plan。
