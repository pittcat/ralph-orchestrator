...
- **SQL migrations v19-v23（DAG store）**：`dag_checkout_intents`（v19，per-Unit capacity 槽位预留）、`dag_terminal_deliveries`（v20，exactly-once close acknowledgement）、`dag_registration_evidence` + `dag_approval_evidence`（v21，correction reentry 持久化）、`dag_correction_requests`（v22，typed artifact_refs 列）、`dag_integration_failures`（v23，`dag_runtime` 虚拟 target 路由表）已落地；新安装自动迁移，旧库由 `ralph loops clean --ledger` + migration runner 升级。详见 `crates/ralph-core/src/supervisor/migrations/`。
