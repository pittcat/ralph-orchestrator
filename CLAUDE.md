...
- Builtin presets: `autoresearch`, `ce-executor-serial`, `ce-executor-lite` (template), `debug`, `merge-loop`(裸 `ce-executor` 已删除:所有 plan-driven 执行请使用 `ce-executor-serial`;仅作模板时可使用 `ce-executor-lite`)
