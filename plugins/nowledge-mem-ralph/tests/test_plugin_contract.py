...
def _plugin_content_files() -> list[Path]:
    ...
    ``.pytest_cache/`` directories are generated by pytest's
    ``cacheprovider`` plugin; they must be excluded too so the plugin
    contract stays green on any developer machine regardless of the
    pytest cache settings. Any hidden dotfile directory (``.gitignore``,
    ``.gitkeep``, ``.vscode``, …) is also excluded: shipped content
    is what the plugin authors add, never the editor's metadata.
    """
    ...
    return sorted(
        ...
        and not any(
            part.startswith(".") for part in path.relative_to(PLUGIN_DIR).parts
        )
    )


def test_plugin_content_excludes_pytest_cache() -> None:
    """``.pytest_cache/`` and other dot-dirs must not appear in shipped content."""
    pytest_cache = PLUGIN_DIR / ".pytest_cache"
    pytest_cache.mkdir(parents=True, exist_ok=True)
    cache_tag = pytest_cache / "CACHEDIR.TAG"
    cache_md = pytest_cache / "README.md"
    cache_tag.write_text("pytest cache sentinel", encoding="utf-8")
    cache_md.write_text("# pytest cache sentinel", encoding="utf-8")
    try:
        listed = {p.resolve() for p in _plugin_content_files()}
        assert cache_tag.resolve() not in listed, (
            "pytest_cache/CACHEDIR.TAG must not appear in shipped content"
        )
        assert cache_md.resolve() not in listed, (
            "pytest_cache/README.md must not appear in shipped content"
        )
        # Regression: regular plugin files still appear.
        assert (PLUGIN_DIR / "scripts" / "memory_marker.py").resolve() in listed
        assert (PLUGIN_DIR / "README.md").resolve() in listed
        # Regression: dotfiles elsewhere are also excluded.
        dotdir = PLUGIN_DIR / ".editorconfig"
        dotdir.write_text("# editor sentinel", encoding="utf-8")
        listed_after = {p.resolve() for p in _plugin_content_files()}
        assert dotdir.resolve() not in listed_after, (
            "dotfile at PLUGIN_DIR root must not appear in shipped content"
        )
    finally:
        cache_tag.unlink(missing_ok=True)
        cache_md.unlink(missing_ok=True)
        (PLUGIN_DIR / ".editorconfig").unlink(missing_ok=True)
        pytest_cache.rmdir()
