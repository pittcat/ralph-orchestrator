...
# troubleshoot and uninstall without reading source.
...
def _plugin_content_files() -> list[Path]:
    """All shipped content files, excluding this test suite."""
    return sorted(
        path
        for path in PLUGIN_DIR.rglob("*")
        if path.is_file() and "tests" not in path.relative_to(PLUGIN_DIR).parts
    )
...
def test_plugin_has_no_lifecycle_entrypoints() -> None:
    """R1/R2/S2 — no hooks in the manifest and none on disk."""
    manifest = _load_manifest()
    assert "hooks" not in manifest, (
        "dedicated plugin manifest must not declare hooks"
    )
    assert not (PLUGIN_DIR / "hooks").exists(), (
        "dedicated plugin must not ship a hooks/ directory"
    )

    # Only markdown resources plus the manifest may be shipped — no
    # executable hook scripts of any kind.
    for path in _plugin_content_files():
        relative = path.relative_to(PLUGIN_DIR)
        assert path.suffix in {".md", ".json"}, (
            f"unexpected non-markdown/json content file in plugin: {relative}"
        )
        if path.suffix == ".json":
            assert relative == Path(".claude-plugin") / "plugin.json", (
                f"only the manifest json may ship in the plugin: {relative}"
            )

    # Registerable surface (manifest + commands + skills) must not even
    # *mention* lifecycle hook events. The README is exempt: it must
    # describe the absence of hooks by name (R7).
    for path in [MANIFEST, *_all_capability_docs()]:
        relative = path.relative_to(PLUGIN_DIR)
        text = path.read_text(encoding="utf-8")
        for event in _HOOK_EVENT_NAMES:
            assert event not in text, (
                f"hook event {event!r} referenced by registerable plugin "
                f"content {relative}; the dedicated plugin must not "
