...
0e. **Key-stage event gate (capability-triggered; per-location guard selection)**:
   ...
   - **逐位置询问菜单**：对每个被识别为关键位置的 handoff / 阶段分支，author 必须用 `AskUserQuestion` 给出 4 选 1:
     1. **加入 `precheck guard`**（推荐当该位置需要主观质量判断）——在 `event_loop.precheck.rules.<topic>` 声明事件级 LLM gate；YAML key 必须是真实会被 emit 的 topic，不要用 `topic.workspace` 后缀另挂一条永远不触发的 rule。可选 `recovery_guidance.common` / `by_check`（precheck key 为 1-based checklist 索引）。producer 先进入 `<topic>.proposed`，合成 gate hat 通过后才发 `<topic>`。它不是 `ralph emit --policy-check`。
...
- **`--triggered`:** 它表示事件目标 hat，不是来源 hat。普通 handoff 省略它，让 isolated 路由自动推导唯一消费者；若 schema 声明 `EventSchema.required_target_hat`，则必须显式指定该目标，包括终态 reporter 的合法 self-target。其它 self-target 属于 P0；目标还必须与唯一消费者和 `policy-check` 一致（见 `references/commands.md`）。
