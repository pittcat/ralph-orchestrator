...
- **`--triggered`:** 它表示事件目标 hat，不是来源 hat。普通 handoff 省略它，让 isolated 路由自动推导唯一消费者；self-target 属于 P0。只有已确认的跨 hat 直达例外才可显式指定不同 hat，并须有 author notes 的原因与拓扑证据（见 `references/commands.md`）。
