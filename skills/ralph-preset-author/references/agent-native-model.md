...
| Loop 外评审 | `skills/ralph-preset-author/references/` + `skills/ralph-preset-review/references/` | 作者 / 评审如何检查分层与 AAF、payload audit 表、confidence 校准（author 与 review 各自持有一份能力一致的副本，覆盖独立安装契约）|
...
- **`skills/ralph-preset-author/references/agent-native-model.md`**（loop 外评审层，本文件）+ `skills/ralph-preset-review/references/agent-native-model.md` 同步副本：写 artifact-first 模型、判定标准（恢复价值 / 审计价值 / 下游依赖）、违规列表、边界（业务 artifact vs internal ledger）、灰色地带判定方法。**不写**具体的「`.ralph/<...>` 子目录命名约定」，由 preset 自决。
...
- `RALPH_DAG_TASK_KEY` — 当前 Unit 对应 task 的注册 key（含 `:step-<n>:` 段）
- `RALPH_DAG_TASK_ID` — 当前 loop 的真实 live task id
- `RALPH_DAG_JOB_ID` — 本次 job 的唯一 id
- `RALPH_DAG_JOB_TOKEN` — 本次 job 的一次性令牌
- `RALPH_DAG_STAGE` — 本轮所处阶段，例如 `execution` / `review` / `verification`
- `RALPH_DAG_ATTEMPT` — 本 Unit 的第几次尝试
