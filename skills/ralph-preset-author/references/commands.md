...
- **builtin 现状**：`parallel-forge` 的 `scheduler_mode: dag` 使用 runtime-owned DAG execution face；author 起草其他 supervisor preset 时仍按上述 fail-closed 组合写配置，并让 hat 只产出业务判断/证据，不伪造 runtime admission、job 或 terminal receipt。
