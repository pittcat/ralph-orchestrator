...
* The generated argv always carries ``-c ralph.<stem>.yml`` /
  ...
  preempt the target suite. Launch argv uses
  ``--prompt-file PROMPT.<stem>.md`` (agent-visible prompt: change
  intent + full workload body) **and** ``--plan docs/plans/<basename>``
  (workload identity / worktree key). Ralph gives ``--prompt-file``
  precedence over ``--plan`` for the loop prompt, so change intent
  reaches agents.
...
# Agent-visible prompt. Ralph ``run --prompt-file`` wins over ``--plan``
# for the loop prompt, so this file MUST embed both change intent and
# the full workload body. ``--plan`` still points at the pure workload
# file for worktree naming / plan identity.
PROMPT_TEMPLATE = """# Ralph E2E bootstrap — preset-bound prompt

This preset-bound suite is generated by ``ralph-e2e-bootstrap``.
Launch argv uses ``--prompt-file`` (this file) plus ``--plan {plan_relpath}``.

## Change intent (orchestrator verification context)

This section is **not** sandbox implementation scope. It records what
the operator is verifying from the orchestrator / current-repo change
plan so agents know why this E2E run exists.

Change plan path: {change_plan_path}
Change plan sha256: {change_plan_hash}

{change_summary}

## Workload plan (sandbox business scenario)

Preset: {preset}
Stem: {stem}
Workload plan (sandbox-relative, also passed as ``--plan``): {plan_relpath}

Do not treat the change-intent section above as files to edit inside
this sandbox. Execute the workload below.
...
def _render_payloads(
    ...
    *,
    ...
    prompt_payload = PROMPT_TEMPLATE.format(
        ...
        change_plan_hash=change_plan_hash,
...
def _build_argv(
    ...
    """Return (dry_run_argv, launch_argv) for the suite.

    Uses sandbox-relative ``ralph.<stem>.yml`` / ``PROMPT.<stem>.md`` so
    the operator can ``cd`` into the sandbox and paste the launch
    command. ``--prompt-file`` carries change intent + workload body;
    ``--plan`` is the pure workload path.
    """
    config_rel = f"ralph.{stem}.yml"
    prompt_rel = f"PROMPT.{stem}.md"
    base = (
        binary,
        "-c",
        config_rel,
...
def generate_suite(
    *,
    ...
    plan_path:
        **Workload** plan path (sandbox-local). Read-only — its bytes
        are hashed once and never modified. This path becomes
        ``ralph run --plan`` (workload identity). Its body is also
        embedded into ``PROMPT.<stem>.md``.
    change_plan_path / change_plan_hash / change_summary:
        **Required** orchestrator **change plan** verification context
        injected into ``PROMPT.<stem>.md`` (not used as ``--plan``).
    stem:
    ...
    SandboxError
        ...
        the plan file is unreadable, or change-plan context is
        missing. The exception message is operator-facing.
    """
    sandbox = Path(sandbox).resolve()
    _check_writable(sandbox)

    if not change_plan_path or not str(change_plan_path).strip():
        raise SandboxError(
            "change_plan_path is required; refuse suite without "
            "verification intent (cannot reach agents via --prompt-file)"
        )
    if not (change_plan_hash or "").strip():
        raise SandboxError(
            "change_plan_hash is required; call resolve_plans first"
        )
    if not (change_summary or "").strip():
        raise SandboxError(
            "change_summary is required; call resolve_plans / "
            "extract_change_summary first"
        )
    ...
    try:
        workload_body = plan_bytes.decode("utf-8")
    except UnicodeDecodeError as exc:
        raise SandboxError(
            f"workload plan is not utf-8: {exc.__class__.__name__}"
        ) from exc

    # Stage the workload plan into the sandbox so the live loop
    # (launched from the sandbox cwd) can find it. argv then uses the
    # staged sandbox-relative path. The source plan bytes remain
    # untouched.
    ...
    change_path_str = str(Path(change_plan_path))
    change_hash_str = change_plan_hash.strip()
    change_summary_str = change_summary.strip()
    ...
    config_payload, prompt_payload = _render_payloads(
        ...
        preset,
        workload_body=workload_body,
        change_plan_path=change_path_str,
        change_plan_hash=change_hash_str,
    ...
    argv, launch_argv = _build_argv(
        binary_token, resolved_stem, preset, staged_plan_relpath
    )
