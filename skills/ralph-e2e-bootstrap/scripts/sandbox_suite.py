...
## Workload plan (authoritative for ``ralph run --plan``)

The loop's primary prompt source is the workload plan below.
Do not treat the change-intent section as implementation scope inside
this sandbox.

Workload plan (sandbox-relative): {plan_relpath}
Preset:    {preset}
Stem:      {stem}

## Change intent (orchestrator verification context)

This section is **not** the ``--plan`` workload. It records what the
operator is verifying from the orchestrator / current-repo change plan
so agents know why this E2E run exists.
...
def _render_payloads(
    ...
    *,
    ...
    change_plan_hash: str = "(none)",
    change_summary: str = "(no change plan supplied)",
) -> tuple[bytes, bytes]:
    ...
    *,
    ...
    plan_path:
        **Workload** plan path (sandbox-local). Read-only — its bytes
        are hashed once and never modified. This is the only path that
        becomes ``ralph run --plan``.
    change_plan_path / change_plan_hash / change_summary:
        Optional orchestrator **change plan** verification context
    ...
    change_path_str = (
        str(Path(change_plan_path)) if change_plan_path else "(none)"
    )
    change_hash_str = change_plan_hash or "(none)"
    change_summary_str = change_summary or "(no change plan supplied)"
