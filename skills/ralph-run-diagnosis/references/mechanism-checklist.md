...
| `scheduler_mode: dag_shadow | dag` | scheduler 观测 + DAG 业务事件 + runtime terminal receipt | 由 runtime-owned receipt / main events 对账 | 不要求 `wave_id`；缺 DAG 证据时按证据缺口记录，不改判为 wave 故障 |
