...
def render_pipeline_yml(
    *,
    ...
    The owned keys (``preset``, ``plan``, ``prompt_file``, ``preflight``)
    live under a top-level ``_bootstrap:`` mapping. Additional user
    keys (e.g. ``event_loop:``, ``budget:``, ``diagnostics:``) sit at
    the same indentation level as ``_bootstrap:`` and are emitted with
    stable indentation.

    The ``project_root_marker`` is a free-form scalar recorded in the
    user-keys block so callers can see which directory the suite is
    anchored on. Defaults to ``./`` which the runtime treats as
    repo-relative.
    """
    if not preset:
    ...
    budget_lines = _render_mapping_lines(
        {
            "max_iterations": str(int(budget_max_iterations)),
            "wall_clock_seconds": str(int(budget_wall_clock_seconds)),
        }
    )
    diagnostics_block = ["diagnostics:", "  enabled: " + ("true" if diagnostics_enabled else "false")]
    header = (
    ...
    lines.extend(diagnostics_block)
