...
- **2026-06-15 — Path A alone was not enough**: After Path A landed, the plan-gate correctly dual-published `queue.advance` + `work.ready` in the same turn, but `isolated` mode's per-turn single-business-event budget in `event_loop/mod.rs:5678` still dropped `work.ready` as `event.isolation.boundary_violation`. The plan-gate then repeated the same pair, the loop hit `consecutive_same_signature >= 3`, and the loop terminated as `LoopStale` (status: `Failed: stale loop detected`, 7 steps uncompleted). Lesson: a preset-level fix that produces a two-business-event sequence in one turn only survives the runtime if the orchestrator's budget admits an exception for that exact ordered pair. Future dual-publish patterns (any other preset, any other pair) must add a corresponding orchestrator-level carve-out before the preset ships; otherwise the fix is a no-op in the bus. See `docs/plans/2026-06-15-003-fix-plan-gate-dual-publish-isolated-budget-plan.md` for the paired fix and `docs/report/2026-06-15-plan-gate-dual-publish-blocking-diagnosis.md` for the failure trace.
...
#### Infrastructure dependency (2026-06-15-003 fix)

Path A only works because the orchestrator's `isolated` mode admits the ordered pair `(queue.advance, work.ready)` as the only legal two-business-event sequence in a single turn. Without that carve-out — the default per-turn single-business-event budget — the second event (`work.ready`) is dropped by `crates/ralph-core/src/event_loop/mod.rs:5678` (`event.isolation.boundary_violation`), the executor never wakes, the plan-gate repeats the same pair, and after `consecutive_same_signature >= 3` the loop terminates as `LoopStale`. This was the 2026-06-15 incident (`docs/report/2026-06-15-plan-gate-dual-publish-blocking-diagnosis.md`) where the preset-level fix from Path A was in place but the runtime budget still dropped `work.ready` — the loop only made progress once the orchestrator-level carve-out landed. Implementation: `is_dual_publish_step_handoff` in `event_loop/mod.rs`, with a narrow, ordered, exact-topic allowlist and the third business event still dropped (sticky budget). New unit tests + a BDD scenario (`tests/scenarios/plan_gate_dual_publish_handoff.yml`) pin the carve-out. The companion `isolated_boundary_violation` scenario continues to enforce that any non-`(queue.advance, work.ready)` second business event is still rejected. When touching this carve-out, run both scenarios — the regression for the third-event drop and the dual-publish acceptance live one test pair away.
