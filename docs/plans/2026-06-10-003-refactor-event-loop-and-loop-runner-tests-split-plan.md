...
- `crates/ralph-core/src/event_loop/mod.rs`（**6 501 行**（baseline @ commit 918192a；2026-06-10 立项时为 5 733 行、2026-06-12 v1 baseline @ 37bd281 时为 6 361 行，v2 推进 +140 行来自 WAC 静态规则 + ce-executor-isolated 闭环），主要是 ~5 522 行的 `impl EventLoop` 块（mod.rs:926-6448））拆为 **9 个新建子文件 + 1 个 `prompt.rs` 内的 process 嵌套 mod**（`payload_contract` ~70 行并入 `policy.rs`；保留**已存在**的 `loop_state` / `rejection` / `review_step_state` / `tests` **四个** mod 声明；U1 scaffold 时预留 10 个 placeholder 但 U6 实施时 process 占位通过 `pub(crate) mod process` 在 prompt.rs 内实现，**不**新增 `process.rs` 文件）。
- `crates/ralph-cli/src/loop_runner/tests.rs`（**11 015 行、192 个测试** + 大量 helper）拆为 **17 个 `.rs`**：1 个 `tests/mod.rs`（保留 mutex 文档 + tests.rs 内**仅 2 个** `FAKE_PATH_BACKEND_*` Mutex；`MOCK_ACP_*` 已迁到 `crates/ralph-cli/src/loop_runner/wave/acp_mock.rs:97-104` 为 `pub static`，本轮**不动**）+ 1 个 `tests/common.rs`（真正跨子文件共享的 helper）+ 15 个主题子文件（user_interactive + pty 合并、output_processing + state_machine 合并等，**0 个子文件 < 200 行**）。
...
`event_loop/mod.rs` 的 ~5 522 行 `impl EventLoop` 块（mod.rs:926-6448，2026-06-12 v2 baseline @ 918192a）尤其难维护——单文件改动 PR 难以快速 review、容易误碰无关逻辑、且子方法之间的领域边界（lifecycle / policy / dispatch / diagnostics）已经清晰可见。
`loop_runner/tests.rs` 把 **192** 个跨多个领域的测试挤在一个文件里，无法按领域局部 review；新加测试时需要 1 万行单文件编辑。
...
零回归意味着**所有 192 个测试（v2 baseline @ 918192a 实测，v1 baseline @ 37bd281 写 193，v2 -1 来自 WAC commit `918192a` 清理 dead code）+ 40 个 `event_loop/tests/` 子文件 + 所有使用 `EventLoop` 的下游代码（`runner.rs` / `mod.rs` / `operation_guard.rs` / `adapters/pty_executor.rs`）必须保持编译并通过全部测试**。
...
- R2. **零行为变化**：所有现有 `#[test]` 函数签名 / 断言 / 输出 / 错误信息**逐字节不变**；`TerminationReason` **16 变体**顺序 + 3 处 `match` 表达式覆盖顺序（`exit_code` mod.rs:168-191 / `as_str` mod.rs:198-215 / `is_success` mod.rs:218-220）不变；`EventLoop` **14 字段**（v2 baseline @ 918192a 实测，v0/v1 baseline 写 13 是早期错算，v2 增量字段为 `hat_lifecycle_tracker: ActivationLifecycleTracker<SystemTimeClock>` 等）顺序不变。
...
- R3. **零公开 API 破坏**：`lib.rs:80` 的 `pub use config::{...}` 列表（v2 baseline，v1 baseline 写 79）、**`lib.rs:104` 的 `pub use event_loop::{...}` 列表**（v2 baseline，v1 baseline 写 103）、`event_loop::*` / `loop_runner::*` 全部 `pub` 与 `pub use` 路径**保持不变**；`runner.rs` / `summary_writer.rs` / `drift/engine.rs` / `diagnostics/integration_tests.rs` / `event_loop/tests/replay_light_integration.rs` 等下游文件不修改 import。
...
- KTD5. **类型/字段顺序敏感（修正伪风险）**：抽 `event_loop/types.rs` 时**整段声明原封不动复制**；`TerminationReason` **16 变体**顺序 + 3 处 `match` 表达式覆盖顺序（`exit_code` 行 168-191 / `as_str` 行 198-215 / `is_success` 行 218-220）不变；`EventLoop` **14 字段**（v2 baseline @ 918192a 实测，含 `pub(crate)` 与辅助字段；v0/v1 baseline 写 13 是早期错算，v2 增量字段为 `hat_lifecycle_tracker: ActivationLifecycleTracker<SystemTimeClock>` 等）顺序不变。**注**：`event_loop/mod.rs` 中**无任何 `#[serde(...)]` / `#[serde(flatten)]` / `#[serde(default)]` / `#[serde(untagged)]` 标注**（grep 验证为空），KTD 早期草稿中的 "serde attribute 位置不变" 是伪风险——已删除。U3 / U4 完成后用 `git diff` 字节级 + `git grep -A 1 "TerminationReason" types.rs | head -50` 验证变体顺序。**baseline 变更说明（v2 校正）**：v0 plan 写 "17 变体 / 14 字段" 是 2026-06-10 立项时的早期错算（v1 时已校正为 16 / 13），v2 实测 **16 / 14**——`EventLoop` 在 v0→v2 期间新增 1 个字段（最可能为 `hat_lifecycle_tracker` 或 `recovery_responder`，v1→v2 之间 `recovery_responder` 由 ce-executor-isolated 闭环 commit `f8887ed`/`fe9ae37` 引入）。U3 / U5 实施前必须**重新跑** `awk '/^pub struct EventLoop/{f=1;next} f && /^}/{exit} f && /^    [a-z_]+:/{c++} END{print c}' crates/ralph-core/src/event_loop/mod.rs` 确认输出 14（v2 baseline）。
- KTD6. **U1→U7 风险递增顺序**：
  ...
  5. **U5**：拆 `event_loop/{lifecycle.rs, termination.rs, dispatch.rs, prompt.rs, diagnostics.rs}.rs`（`impl EventLoop` 块按方法域分块；**`process_parse_result` 1 618 行单方法归 `prompt.rs`**（v2 baseline mod.rs:4530-6147，v1 baseline 1 564 行，v2 +54），因 prompt 域是它主语义归属）—— 主体改动
...
- KTD12. **5 个 `impl EventLoop` 域的边界规则**（U5 实施时遵循）：
  ...
  - **prompt.rs**：方法主返回是 `UserPrompt` / `process_parse_result` / 涉及 prompt 构建与解析的，归属 prompt（**`process_parse_result` 1 618 行整体归 prompt**（v2 baseline mod.rs:4530-6147；v1 baseline 1 564 行 → v2 +54 行），因它是 prompt 解析主入口）
...
- KTD13. **6-03-002 KTD 对照表**（平移 + 调整）：
  ...
  | KTD5 serde 顺序 | KTD5 | **修正**：删除 `#[serde(flatten)]` 伪风险，改为 `TerminationReason` **16 变体** + 3 处 match + `EventLoop` **14 字段**顺序（**v2 baseline @ 918192a 实测**，v0 立项 17 / 14 中 17 为早期错算，14→13 由 v1 doc-review 触发，**v2 校正回 14**——`hat_lifecycle_tracker` / `recovery_responder` 等 v0→v2 期间增量字段） |
...
flowchart TB
  subgraph event_loop["event_loop/"]
    ...
    prompt["prompt.rs<br/>≈ 3 260 行<br/>impl EventLoop prompt 处理<br/>+ process_parse_result 1 618 行<br/>🆕 新建"]
...
**预估行数依据（v2 baseline @ 918192a）**：mod.rs 当前 6 501 行 - types 230 - workflow_guard 260 - policy 600 = ~5 411 行属于 `impl EventLoop` 块，按 lifecycle 1 320 + termination 920 + dispatch 660 + prompt 3 260 + diagnostics 390 = 6 550 行（含 use 段重复 + 跨域方法的小量复制冗余 ~1 140 行，符合实际多文件 impl 拆分的开销；v1 baseline 行数估 6 430 行，v2 +120 行）。
...
**预估行数依据（v2 baseline @ 918192a）**：tests.rs 当前 11 015 行 / 192 个测试（v1 baseline 10 993 行 / 193 个测试，v2 +22 行 / -1 测试——WAC commit `918192a` 清理 dead code），按主题聚类的平均规模 50-60 行/测试 + helper（fake_path 占 220、wave 占 2 220、hooks 占 1 820、其余按行数分布）。
...
- **Goal**: 把 `crates/ralph-core/src/event_loop/mod.rs:59-280`（baseline 2026-06-10 行号；2026-06-12 v1 重新定位为 **mod.rs:59-281**；2026-06-12 v2 @ 918192a 重新定位为 **mod.rs:59-287**，包含 `ProcessedEvents` / `ProcessedEventsWithWaves` / `TerminationReason` 枚举 + `impl TerminationReason` + `WorkflowGuardRejection`；v2 中 mod.rs:288 为 `extract_correlation_key` 起点）抽到 `crates/ralph-core/src/event_loop/types.rs`。U5 实施前必须**重新跑** `grep -nE "^pub (enum|struct) (ProcessedEvents|ProcessedEventsWithWaves|TerminationReason|WorkflowGuardRejection)" crates/ralph-core/src/event_loop/mod.rs` 确认行号未漂移。
- **Requirements**: R1 / R2 / R3 / R5
- **Dependencies**: U1；U2（分离关注点）
- **Files**:
  - 修改 `crates/ralph-core/src/event_loop/types.rs`（从 placeholder 改为实质内容，~230 行）
  - 修改 `crates/ralph-core/src/event_loop/mod.rs:59-288` 段（删除 + 顶部 `pub use types::{ProcessedEvents, ProcessedEventsWithWaves, TerminationReason};`；v2 baseline 288 行为 `extract_correlation_key` 起点，比 v1 baseline 281 +6 / 比 v0 立项 280 +8）
- **Approach**:
  1. 整段（动手前重新 grep 定位）原封不动复制到 `types.rs`
  2. `mod.rs` 顶部加 `pub use types::{ProcessedEvents, ProcessedEventsWithWaves, TerminationReason};`
  3. `mod.rs` 删除原 59-287 段（v2 baseline）
...
- **Goal**: 把 `crates/ralph-core/src/event_loop/mod.rs:282-925`（baseline 2026-06-12 v2 @ 918192a，实测：`extract_correlation_key` 288 / `apply_workflow_guard_validation` 371 / `apply_event_policy_validation` 551 / `finding_to_payload_contract_violation` 857-925；v1 baseline 行号 +6 整体偏移）的自由函数抽到 2 个新子文件（`payload_contract` ~70 行内容并入 `policy.rs`）。U4 实施前必须**重新跑** `grep -nE "^fn (extract_correlation_key|apply_workflow_guard|apply_event_policy|finding_to_payload_contract)" crates/ralph-core/src/event_loop/mod.rs` 确认行号未漂移。
- **Requirements**: R1 / R2 / R3
- **Dependencies**: U3
- **Files**:
  - 修改 `crates/ralph-core/src/event_loop/workflow_guard.rs`（~280 行：`extract_correlation_key` + `WorkflowGuardRejectionDetail` + `WorkflowGuardOutcome` + `apply_workflow_guard_validation`，对应 mod.rs:288-550）
  - 修改 `crates/ralph-core/src/event_loop/policy.rs`（~620 行：`apply_event_policy_validation` + `finding_to_payload_contract_violation` + 相关 helper，对应 mod.rs:551-925）
  - 修改 `crates/ralph-core/src/event_loop/mod.rs:288-925` 段（删除 + 顶部 `pub use` 重新导出）
- **Approach**:
  1. 按函数段整段复制：288-550 → `workflow_guard.rs`；551-925 → `policy.rs`（含原 857-925 payload_contract 段）
  2. 函数内 `use crate::*` 引用按需保留或重写为 `use super::*`（如果跨子模块）
  3. `mod.rs` 顶部加 `pub use workflow_guard::*; pub use policy::*;`
  4. `mod.rs` 删除原 288-925 段
...
- **Goal**: 在 U5 实施 **1 618 行** `process_parse_result`（v2 baseline mod.rs:4530-6147，v1 baseline 1 564 行 → v2 +54）整体归 `prompt.rs` 之前，先在 plan 中显式列出 6 个 inline validation 层（origin guard / topic format / event policy / state machine / workflow guard / execution contract）的 KTD12 归属矩阵，让 reviewer 提前判定是否真的能按"主返回 / 主副作用归属 prompt 域"统一处理；如有歧义记入 R3 follow-up 而非在 PR 中临时判定（避免 R-Refactor-9 "U5 主体改动 1 618 行方法归错域后无法在不动业务的前提下重新分块" 风险）。
...
- **Goal**: 把 `crates/ralph-core/src/event_loop/mod.rs:926-6501`（baseline 2026-06-12 v2 @ 918192a，实测：`impl EventLoop` 主块 926-6448（约 5 522 行）；尾部 `format_duration` 6458 / `termination_status_text` 6474 作为自由函数随 `termination.rs` 一并迁出）的 ~5 522 行按方法域拆到 5 个新子文件（lifecycle / termination / dispatch / prompt / diagnostics）；**`process_parse_result` 1 618 行单方法（mod.rs:4530-6147）整体归 `prompt.rs`**（按 KTD12 边界规则：方法主语义是 prompt 解析；v2 比 v1 baseline +54 行）。**新增方法注意**：commit `37bd281` 引入的 `inject_review_aggregate_timeouts` 归 **diagnostics**（KTD12 规则：返回 bool + 副作用是 telemetry / 注入 recovery，diagnostics 域）；`review_step_tracker.check_semantic_gates` / `observe_accepted` 由 `review_step_state` 模块自管，不在本拆分范围。U5 实施前必须**重新 grep** `impl EventLoop` 起止 + 所有 117 个方法名（`awk '/^impl EventLoop/{f=1;next} f && /^}/{exit} f && /^    (pub )?fn /{c++} END{print c}'`，2026-06-12 v2 baseline 实测输出 117，与 v1 baseline 一致），确认与 KTD12 规则对应一致。
- **Requirements**: R1 / R2 / R3
- **Dependencies**: U4
- **Files**:
  - 修改 `crates/ralph-core/src/event_loop/lifecycle.rs`（~1 320 行：生命周期方法 + KTD12 规则；v2 比 v1 baseline 估 +20）
  - 修改 `crates/ralph-core/src/event_loop/termination.rs`（~920 行：终止条件方法 + `format_duration` + `termination_status_text` + KTD12 规则；v2 +20）
  - 修改 `crates/ralph-core/src/event_loop/dispatch.rs`（~660 行：调度方法 + KTD12 规则；v2 +10）
  - 修改 `crates/ralph-core/src/event_loop/prompt.rs`（~3 260 行：prompt 处理方法 + **`process_parse_result` 1 618 行整体方法体**；v2 prompt 处理 +60 / process_parse_result +54）
  - 修改 `crates/ralph-core/src/event_loop/diagnostics.rs`（~390 行：诊断方法 + `inject_review_aggregate_timeouts` + KTD12 规则；v2 +10）
  - 修改 `crates/ralph-core/src/event_loop/mod.rs:926-6501` 段（删除整块 impl + 末尾两个自由函数；v2 baseline mod.rs 6 501 行）
- **Approach**:
  ...
  5. `mod.rs` 删除原 926-6501 整段 impl + 末尾两个自由函数
  6. `git diff` 验证每个方法体字节级未变（`diff -u <(sed -n '926,6501p' mod.rs.bak) <(cat lifecycle.rs termination.rs dispatch.rs prompt.rs diagnostics.rs)`）
...
- **Test scenarios**:
  ...
  - **Method-level byte preservation**: 5 个子文件方法体总字节 = 原 `mod.rs:926-6501` 段
  - **Method visibility preserved**: **117 个方法**（2026-06-12 v2 baseline 实测，与 v1 一致）的 `pub` / `pub(crate)` / private 可见性逐个未变
...
- **Verification**:
  - `cargo nextest run --workspace --exclude ralph-e2e --no-fail-fast` 全绿
  - `wc -l crates/ralph-core/src/event_loop/{mod,lifecycle,termination,dispatch,prompt,diagnostics}.rs` 显示 `mod.rs` 减约 5 522 行，5 个新子文件总和 ~6 550 行（v2 比 v1 baseline 估 +120）
...
- 任何公开行为、错误信息、日志、退出码、`TerminationReason` **16 变体**顺序、`EventLoop` **14 字段**（v2 baseline）顺序改动（U6 字节级改写除外）
...
- **R-Refactor-2**（高）：`EventLoop` **14 字段**顺序在 U5 拆 impl 时漂移 → `Default` / `Debug` / `PartialEq` 派生输出变化。**检测机制**：U5 完成后用 `git diff` 字节级 + `awk '/^pub struct EventLoop/{f=1;next} f && /^}/{exit} f && /^    [a-z_]+:/{c++} END{print c}' crates/ralph-core/src/event_loop/mod.rs` 输出 **14** 验证（修正：v0 baseline 写 13 是早期错算，v1 baseline 也写 13 是 doc-review 触发后仍未补正，**v2 baseline @ 918192a 实测 14**——新增字段为 `hat_lifecycle_tracker: ActivationLifecycleTracker<SystemTimeClock>` 及其他 v0→v2 期间增量；`-A N` 是后 N 行上下文，与字段数无关）。Escape hatch 同 R-Refactor-1。
...
- **lib.rs:104 stable**（v2 baseline 修正；v1 baseline 写 103，v2 +1）：`git diff HEAD~1 -- crates/ralph-core/src/lib.rs | grep -E "^\+.*pub use event_loop::|^-.*pub use event_loop::"` 0 输出
- **TerminationReason order**: `git grep -A 1 "TerminationReason" crates/ralph-core/src/event_loop/types.rs | head -50` **16 个变体**顺序未变
- **EventLoop field order**: `awk '/^pub struct EventLoop/{f=1;next} f && /^}/{exit} f && /^    [a-z_]+:/{c++} END{print c}' crates/ralph-core/src/event_loop/mod.rs` 输出 **14**（v2 baseline @ 918192a 实测；v0/v1 baseline 写 13 是早期错算）+ `git diff HEAD -- crates/ralph-core/src/event_loop/mod.rs` 字节级验证字段顺序未变（修正：原 plan `git grep -A 14` 把 14 误当字段数；`-A N` 是后 N 行上下文，与字段数无关）
...
- **模式与组织（已存在）**：
  ...
  - `crates/ralph-core/src/lib.rs:80`（v2 baseline，`pub use config::{...}` 公开 API 列表起点；v1 baseline 写 79，v2 +1；**禁止修改**）
  - `crates/ralph-core/src/lib.rs:104`（v2 baseline，`pub use event_loop::{...}` 公开 API 列表起点；v1 baseline 写 103，v2 +1；**禁止修改**）
  ...
  - `crates/ralph-core/src/event_loop/review_step_state.rs:1-605`（v2 baseline，commit 37bd281 引入，v1 baseline 512 行 → v2 605 行 +93；本计划**不动**）
...
- **项目硬约束（不可改）**：
  ...
  - `lib.rs:80` 的 `pub use config::{...}` 公开 API 列表（v2 baseline）
  - `lib.rs:104` 的 `pub use event_loop::{...}` 公开 API 列表（v2 baseline）
...
- **目标文件 baseline**（**2026-06-12 v2 @ commit 918192a**；v1 baseline @ 37bd281 已废止；2026-06-10 立项 baseline 已废止）：
  - `crates/ralph-core/src/event_loop/mod.rs`（**6 501 行**；v1 6 361 行 / 立项 5 733 行；v1→v2 +140 行来自 WAC 静态规则 + ce-executor-isolated 闭环）
  - `crates/ralph-cli/src/loop_runner/tests.rs`（**11 015 行、192 个测试**；v1 10 993 行 + 193 个测试 / 立项 9 891 行 + 153 个测试；v1→v2 +22 行 / -1 个测试，可能来自 WAC commit `918192a` 的 dead_code 清理）
  - `crates/ralph-cli/src/loop_runner/runner.rs`（170 KB，本轮**不拆**）
  - `crates/ralph-core/src/config/ralph_config.rs`（3 660 行，本轮**不拆**）
  - **已存在子模块**：`crates/ralph-core/src/event_loop/review_step_state.rs`（**605 行**，commit 37bd281 引入，v1 baseline 512 行 → v2 605 行 +93），本计划**不动**
...
（U3-U6 每 U 完成时追加 Sub-Note；U7 合并为完整表格。模板参考 `docs/plans/2026-06-03-003-refactor-schema-refs-replace-regex-plan.md` 的 "Repo Drift Note" 段。v2 baseline refresh 段已就地追加在 v1 段之后。）
