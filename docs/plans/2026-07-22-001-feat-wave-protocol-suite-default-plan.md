...
related_plans:
  - docs/plans/2026-07-22-002-feat-preset-skills-execution-model-wave-supervisor-plan.md
  - docs/plans/2026-07-22-004-feat-payload-consistency-gates-plan.md
  - docs/plans/2026-07-22-003-fix-supervisor-worktree-dispatch-plan.md
---
...
- Authority: 本计划 Product Contract + KTDs；冲突时以本计划为准。与 `docs/plans/2026-07-23-001-fix-supervisor-worktree-dispatch-closure-plan.md`（下称 **023-closure**）冲突时：**supervisor.enabled 生产闭合优先于本计划 U2+**；本计划不得回退 023 的 pipeline 非干扰回归门禁，但会在「有 wave 且 `enabled=false`」路径上**有意扩展** store 语义（见 KTD-1 / Baseline）。
- Sequencing: **U1（wave_verify_gate）可与 023-closure 并行**；**U2–U9 默认在 023-closure 生产 bridge/dispatch/fan-in 闭合后启动**，复用其 store/bridge API，禁止另写第二套账本或假阳性 helper。
- Stop when: Verification Contract 全部绿灯；非目标（P2 fail-fast / 增量 emit / 成本帽 / 改 pipeline 拓扑）未潜入 diff。
- Out of scope reminder: 不把 wave 升为 CE 主路径；不替代 executor 内 subagent；不吸收 004 payload-consistency 引擎；不重做 002 operator-skills 执行模型文档（仅 agent 注入 `data/*.md`）。

Product Contract preservation: 本计划为 `ce-plan-bootstrap` 直接规划；会话确认 `1A / 2B / 3A` 已写入 KTDs。2026-07-23 rebaseline 不改变 Product Contract ID，只改 Planning/Unit 相对当前代码的落地路径。

---

## Baseline Audit（2026-07-23）

> 审计基准：分支 `pittcat-dev`；相关提交含 `45dd03d2`（本计划初稿）、`28a49bbc`/`09903aa1`/`6b371d92`/`71f3926e`（旧 003 U1–U4）、`149cf6c0`（023-closure 取代 003）、`f293bd47`…`e8a7f0a2`（002 operator skills）。
...
| DB 路径无双 `.ralph` | **已落地** | `resolve_supervisor_db_path` + `wave_supervisor` 路径测。复用即可 |
| `SupervisorStore` 六件套 API（enqueue / try_dispatch / cancel / content_hash / compensation 表） | **API 存在；补偿执行仍半成品** | `memory.rs` / `rusqlite.rs` / `memory_protocol_tests`；`CompensationEntry` 等多处 `dead_code`；recover 仍有「不跑补偿」注释向 |
| `supervisor.enabled=true` 生产 bridge | **023-closure 在途；旧 U4 假绿** | `build_supervisor_bridge` → `from_store`（`context: None`）→ Exec/Fix `bind_slot` 得 `Ok(None)` → 主工作区 spawn。**禁止本计划抢写同一接线，应等 023 闭合后复用** |
| 默认 wave（`enabled=false`）账本 | **仍为 legacy `WaveTracker`** | `dispatcher.rs`：`supervisor_bridge` 为 `None` 时独走 `WaveTracker`；runner 注释仍写 R3/KTD-7。**本计划核心缺口仍在** |
| register 失败回退 legacy | **仍存在（本计划须删）** | `execute_wave_via_supervisor`：`register_wave_if_absent` Err → warn + 再进 `execute_wave_structured` legacy |
| 幂等 SSoT | **仍为 CLI sidecar** | `wave.rs`：`.<basename>.idempotency.jsonl`；skill 仍描述 sidecar |
| wave OPAC ticket gate | **未做** | 全仓无 `wave_verify_gate`；仅有 `wave verify` + skill 纪律 + `task_verify_gate` 可仿 |
| 取消杀进程 | **部分** | worker timeout 可 kill；store `cancel_wave` + 跨 wave 显式 cancel 与默认路径未统一 |
| 反压 / worktree 写隔离 | **store/协议侧有；默认路径无；supervisor 生产路径归 023** | 默认路径不读 `max_concurrent_workers` / `isolation_mode` worktree |
| pipeline 无 wave 非干扰 | **已成硬回归（023 R1）** | `bridge_build_invocations` + disabled 无 `supervisor.db`。本计划惰性开店必须保住「无 wave → 零 DB」 |
| CONCEPTS「六件套」词条 | **已有** | `CONCEPTS.md` wave protocol suite；U8 只需对齐最终行为，不必新建词条 |
| 002 operator skills 执行模型 | **已落地（独立范围）** | 本计划 U8 **不**重写 `skills/ralph-preset-*`；只改 agent 注入 `crates/ralph-core/data/*.md` |
| 004 payload consistency | **正交并行** | 挂 `event_policy.payload_consistency`；不替代本计划 R9 ticket gate |
| HARD RULE 5 hat env scrub | **已入库** | 新增/改 spawn `ralph` 的测必须 `scrub_agent_runtime_env` / `common::ralph_bin()` |

### 假绿 / 双账本风险（写入 U2）

```text
今日分叉:
  supervisor.enabled=true  → bridge(+store)   [023 正在修生产空转]
  supervisor.enabled=false → WaveTracker only [本计划要吸收六件套]

本计划落地后目标:
  任意 DetectedWave → SupervisorStore 热路径
  supervisor.enabled 仅保留「全量 supervisor preset 语义」
    （协调 topic / 默认 worktree 策略 / 虚拟 supervisor consumer / inspect 扩展块）
...
- `crates/ralph-cli/src/loop_runner/wave/{dispatcher,supervisor_bridge}.rs`（在 023 闭合后的生产接线之上扩展默认路径）
- `crates/ralph-cli/src/task_verify_gate.rs`（ticket 行为结构模板）
- `memory_protocol_tests` / 既有 `wave_supervisor` characterization
...
| ID | 能力 | 外部可观察结果 | 相对 2026-07-23 基线 |
|----|------|----------------|----------------------|
| R1 | 默认 wave 走统一协议 store（非仅内存 `WaveTracker` 孤岛） | 任意 preset 触发 wave 时，ledger 可查询 slot/wave 状态 | **仍缺口** |
| R2 | 状态持久化 + 启动恢复 | 杀进程重启后未完成 wave 可恢复，不重复 inject `*.wave.complete` | feature/默认 DB **已有**；**默认路径 recover 未接** |
| R3 | 分布式取消 | `cancel` / 超时收摊后 in-flight worker 被终止 | **部分**；须统一到默认路径 |
| R4 | 跨 wave 反压 | 达 `max_concurrent_workers` 时新 wave 入队，slot 释放后 FIFO 出队 | store API **有**；默认路径 **无** |
...
| R8 | 写隔离 | `isolation_mode=worktree` 的 wave 每 slot 绑定独立 worktree | **归 023 生产接线后复用**；默认路径须显式接上 |
...
- 不实现 004 的 `payload_consistency` 引擎；不重做 002 的 operator skill 矩阵。
- 不在 023-closure 未闭合生产 bridge 前，用本计划「另造默认路径 worktree factory」绕过假绿。

### 跨计划边界

| 计划 | 关系 |
|------|------|
| 023-closure | **前置依赖（U2+）**；闭合 `enabled=true` 生产链路。本计划吸收六件套到**默认 wave**，复用其 bridge/store，不抢修假绿 U4 |
| 003（superseded） | 仅历史；勿再按旧 U5–U13 执行清单 |
| 002 execution-model skills | 已落地；operator 侧引用其词汇。本计划 U8 只动 `crates/ralph-core/data/*.md` |
| 004 payload-consistency | 正交；同属 Precheck 层但不共享 ticket/store |
...
| KTD-6 | **写隔离默认**：未声明 `isolation_mode` 时 = `shared_readonly`；仅显式 `worktree` 才绑 worktree。实现**复用** 023 闭合后的 `bind_slot` / `worktree_bind`，禁止测试专用 factory 充当生产证据 | 避免静默改变只读 review；防假绿 |
...
| KTD-9 | **反压**：复用 `max_concurrent_workers`（可挂 `event_loop.supervisor` 或 `event_loop.wave` 别名）；跨 wave FIFO `wave_queue`；接线复用 store `enqueue_wave` / `try_dispatch_next` | 复用协议测 |
| KTD-10 | **测试入口**：一律 `cargo nextest`；默认路径覆盖 `supervisor-db`；spawn 测 scrub HARD RULE 5 | HARD RULE 1+5 |
| KTD-11 | **与 023 排序**：U1 可并行；U2+ 默认等 023 生产 bridge/dispatch/record/fan-in 达到「可复用」门槛后再开，避免双写 dispatcher | 防合并冲突与假绿叠加 |
...
flowchart TB
  ...
  subgraph gates [Preserved from 023]
...
- 023-closure 会先把 `enabled=true` 路径上的 bind/dispatch/record/fan-in 做成可复用生产接线；本计划默认路径应调用同一套 API，而不是复制一套。
...
- OQ4. 若需在 023 未完全收尾前启动 U2：仅允许「InMemory 统一账本 + 禁止碰 production worktree factory」的窄切片，并在 scratch 注明与 023 的接口冻结点——默认仍等 023。
...
执行纪律：Unit N 的验收、相关集成、受影响回归全部绿灯后，才能开始 Unit N+1。**本计划内**禁止并行开发多个 Unit（U1 与 023-closure **跨计划**并行除外，见 Goal Capsule / KTD-11）。

**全局前置：** U2–U9 默认要求 023-closure 已把生产 `build_supervisor_bridge` / `bind_slot` / store dispatch·record·fan-in 接到可复用状态（或执行者在 OQ4 窄切片下显式记录冻结接口）。U1 无此前置。
...
- 明确禁止依赖的未来能力：SupervisorStore、持久化、取消、反压、023 生产接线。
...
- 风险：ticket 路径勿进 git。
- Execution note: Implement test-first；可与 023-closure **并行**。

### U2. 默认路径统一到 SupervisorStore（内存先行）

- Unit 目标：`supervisor.enabled=false` 但存在 `DetectedWave` 时，不再独走裸 `WaveTracker` 作为唯一账本；使用 `InMemorySupervisorStore`（或与 023 同构的 bridge facade）注册/记账。
- 对应 Scenario：为后续恢复/反压铺契约；本 Unit 外部可见 = 测试可读取 store snapshot。
- 外部可观察结果：wave 生命周期写入 store phases；legacy 行为 differential 对齐；**无 wave 的 pipeline 仍不建 store/DB**。
- 可依赖：U1；023 可复用的 bridge/store 表面；现有 `memory_protocol_tests`。
- 禁止依赖：强制改 pipeline YAML；在 023 假绿未修时宣称 worktree 生产完成。
- 验收测试：扩展 `loop_runner/tests/wave.rs` + `wave_supervisor.rs`（**enabled false + 有 wave** 也有 store；**enabled false + 无 wave** 仍零 bridge 调用）。
- 单元测试：register / fan_in 计数；与旧 WaveTracker 关键断言 differential。
- Red：默认路径仍无 store。
- 最小实现：`dispatcher.rs` / runner 在 DetectedWave 时构造 store；**删除 register 失败 fallback legacy**（改为错误上抛）。
- 集成验证：既有 wave 测全绿 + 新 store 断言 + pipeline 非干扰回归。
- 回归：`event_loop/tests/wave_*.rs`、flow_reliability wave scenarios、023 R1 门禁。
- 完成标准：R1 在内存语义下成立；无行为回退洞。
- 风险：大 diff；保持 shared_readonly 默认。
...
- 可依赖：U2；已落地的 `default = ["supervisor-db"]` 与路径解析。
...
- Red：recover 仍仅挂在 `enabled=true` 分支。
...
- Red：timeout 只合成事件不杀进程 / 默认路径不调 `cancel_wave`。
...
- 可依赖：U2–U4；store 既有 `enqueue_wave` / `try_dispatch_next`。
- 禁止依赖：补偿、worktree。
- 验收测试：复用/扩展 `memory_protocol_tests`；CLI idempotency 测改为 store；merge io 测。
- Red：超并发仍立刻 spawn；sidecar 与 store 双写不一致。
- 最小实现：接线 enqueue/dispatch；删或 deprecate sidecar；merge 读 content_hash。
...
- Unit 目标：`isolation_mode=worktree` 时 per-slot worktree；默认 shared_readonly 不变；**复用 023 生产 bind**，禁止 helper-only 假绿。
- 对应 Scenario：worktree 写隔离。
- 外部可观察结果：公开 cwd/env；双 slot 文件隔离。
- 可依赖：U2–U3；023 已闭合的 production `bind_slot` / context 注入。
- 禁止依赖：改 pipeline preset；用 `with_context_and_factory` 测试构造器冒充 runner 接线。
- 验收测试：经 runner/`build_supervisor_bridge`（或默认路径等价入口）的 dispatcher 集成，断言真实 cwd。
...
- **不在本 Unit**：重写 `skills/ralph-preset-*`（002 已覆盖）；若 inspect 字段变更需同步 operator 文档，仅做最小交叉引用。
...
- Red：场景期望事件不存在。
- 最小实现：2–4 个 scenario（confirm gate 可用 CLI 测为主；恢复/反压/取消至少各 1）。
- 回归：`ralph-cli` wave*；`ralph-core` scenarios；preset_lint；doctest 按惯例；HARD RULE 5 污染复跑抽样。
...
与上节 U1–U9 一一对应。执行时仅按 Unit 序号串行（U1 可与 023 并行）。

| U-ID | 名称 | 依赖 | 相对基线备注 |
|------|------|------|--------------|
| U1 | wave_verify_gate | — | 全新；可并行 023 |
| U2 | 默认路径 SupervisorStore | U1；023 可复用表面 | 核心缺口 |
| U3 | 默认路径 rusqlite + recover | U2 | feature 已默认；改接线 |
| U4 | cancel + kill | U2, U3 | 统一出口 |
| U5 | 反压 + 幂等 + content_hash | U2–U4 | 删 sidecar |
| U6 | 补偿执行 | U4, U5 | 清 dead_code |
| U7 | worktree 隔离 | U2–U3 + 023 bind | 禁假绿 |
| U8 | inspect + agent skills | U1–U7 | 不动 002 operator 大改 |
| U9 | BDD + 全量回归 | U1–U8 | |
...
- `crates/ralph-cli/src/loop_runner/wave/dispatcher.rs` — `execute_wave_via_supervisor` / legacy 分叉（本计划要收拢）
- `crates/ralph-cli/src/loop_runner/runner.rs` — `build_supervisor_bridge` / recover 接线（等 023 闭合后扩展）
- `docs/plans/2026-07-23-001-fix-supervisor-worktree-dispatch-closure-plan.md` — 生产接线与假绿禁令
...
- [ ] 023 R1 非干扰门禁仍绿；本计划未引入第二套账本
- [ ] 未验证内容与剩余风险已记录：P2 全项、Windows 杀进程、补偿 hook 幂等深化、WaveTracker 类型完全删除
...
- 与 023-closure 并行开发若违反 KTD-11，易出现 dispatcher 合并冲突——应用排序规避。
...
- 无 P2 范围潜入；无 CE 主路径拓扑改动；未与 023 抢修假绿接线。
...
- `docs/plans/2026-07-23-001-fix-supervisor-worktree-dispatch-closure-plan.md`（前置依赖 / 假绿审计）
- `docs/plans/2026-07-22-003-fix-supervisor-worktree-dispatch-plan.md`（superseded，仅历史）
- `docs/plans/2026-07-22-002-feat-preset-skills-execution-model-wave-supervisor-plan.md`（operator skills，正交）
- `docs/plans/2026-07-22-004-feat-payload-consistency-gates-plan.md`（正交 Precheck）
...
- `crates/ralph-cli/src/{wave.rs,task_verify_gate.rs,loop_runner/wave/dispatcher.rs,loop_runner/runner.rs}`
...
- 2026-07-23 rebaseline 代码证据：`dispatcher.rs` legacy 分叉 / register fallback；`Cargo.toml` default features；`wave_supervisor.rs` R1 门禁；全仓无 `wave_verify_gate`
