...
- 状态：`READY`；共同基线：`87c88317c94ce5f15d3e17b74755ade3f3b56a47`；全量基线已验证通过。
- 调查范围：`commands/emit.rs` 生产入口、schema_view、7 个测试族、policy_check 调用关系。
- 已执行：当前行数/符号/测试 mod 扫描、目录先例与 Git 状态检查；`./scripts/run-tests.sh` 通过（Phase 1 7576/7576、Phase 2 23/23、doctest 19/19，4 ignored，退出码 0）。
...
保持 `ralph emit` 的参数、文件输入、policy gate、recovery envelope、JSON/文本输出和 `commands::emit::schema_view` 路径不变，将 6,652 行的 `crates/ralph-cli/src/commands/emit.rs` 拆成根模块、生产实现、schema_view 和测试模块。非目标是改变事件协议、policy 规则、输出字段或权限。

## 2. 代码库现状与证据

生产入口约 1–792 与 856–2,204；测试包含 `emit_reject_summary_tests`、大型 `tests`、4 个 JSON/recorded 测试族；尾部为 `pub mod schema_view`。`emit.rs` 直接 import `policy_check`，但本计划不修改 policy_check。

| ID | 来源 | 观察 | 影响 | 可靠性 |
|---|---|---|---|---|
| E1 | 当前源码/`wc -l` | 6,652 行，生产、测试、schema_view 混合 | 目标确认 | 高 |
| E2 | 顶层 `rg` | 生产入口和 6 个命名测试族边界可定位 | 搬移表 | 高 |
...
4. 基线：生产 1–792/856–2,204；大型 tests 约 3,713 行；schema_view 在尾部。
...
`cargo nextest list --workspace`；`cargo build --workspace`；`cargo nextest run -p ralph-cli --bin ralph -- emit`；`just fmt-check`；`just lint`；`./scripts/run-tests.sh`。失败均停止。
