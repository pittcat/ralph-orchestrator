...
When `ralph-project-bootstrap` generates a pipeline suite for an
...
|-----|------|------------------|-----------------|
| `generator_signature` | string | A `name@version` stamp identifying the bootstrap generator (currently `ralph-project-bootstrap@0.1.0`). | When the bootstrap skill is upgraded. |
| `input_signature` | string | A digest over the inputs the suite was generated against (the catalog, fixture sources, and operator-selected preset/plan identifiers). | When any input changes. |
...
1. **Generator upgrade.** Bumping the bootstrap skill's
   `generator_signature` requires a clean re-emission so the new
   generator is the one stamping the owned block.
2. **Input drift.** Audit results, fixture sources, or the operator
   preset / plan choice changed since the last bootstrap. Run
...
- `ralph.pipeline.yml` — runtime hat configuration (preset, plan,
  prompt file, preflight). Owned block written by bootstrap.
- `PROMPT.pipeline.md` — the per-loop prompt rendered from
  `ralph.pipeline.yml`. Owned block written by bootstrap.

Both files have their SHA-256 listed in `ralph.bootstrap.yml`
`summary`. If bootstrap refuses to refresh and the inputs have not
...
- [ ] Confirm `generator_signature` matches the running bootstrap
