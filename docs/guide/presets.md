...
| `merge-batch` | reviewer → integrator → stabilizer (self-loop) → reporter | Git-first batch merge across worktrees | Isolated multi-hat; emits `merge.integrated` / `merge.stabilized` with byte-stable scope manifest + scope handoff guard |
