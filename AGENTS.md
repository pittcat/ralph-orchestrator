...
- Builtin presets: `autoresearch`, `implementation-review` (6-hat wave review→fix-plan, no supervisor), `ce-executor-pipeline` (14+3 hat 线性一条龙 **Ralph primary path**: plan-reviewer → executor(每个 U-ID 一个 subagent,主 executor 负责验收/提交/最终 emit) → `test-stabilizer`(executor 后一次性测试稳定化门禁,可改测试+生产代码,带 correction ID 与全量门禁) → 6 串行维度 hat → review-synthesizer → fix-planner → fixer(修复一轮并跑全量测试) → alignment → reporter; plan 2026-07-29-001 起 `work.failed`/`fix.done`、plan 2026-07-30 起 `work.done` 共挂 3 个合成 precheck gate hat——done/failed 对称:成功路径(`work.done` / `fix.done` applied)走轻量落地核验(核验 completed Units 真有交付 commit + 验证状态诚实,语言无关),失败路径(`work.failed` / `fix.done` partial|blocked)走完整证据审计;证据不足打回 producer,3 次耗尽 runtime 直发 plan.blocked{kind=precheck_exhausted};成功路径另叠加 5 条确定性 payload_consistency 结构矛盾门禁(work-done complete/commit 一致性 + fix-done applied 一致性,emit 时零 LLM 成本)), `ce-executor-pipeline-loop` (`ce-executor-pipeline` 的环形 review/fix 版本: work.done → `test-stabilizer` → review-reentry → 6 串行维度 review → review-gate；fix.done → review-reentry 直接进入下一轮复审；按 `must_fix_now` 收敛门控继续修复,baseline / 范围外 / 新发现残留进入报告,最多 6 轮), `ce-executor-supervisor` (2026-07-03-001 plan + 2026-07-23-001 closure plan + **2026-07-23-005 重构 + 2026-07-24-001 三 plan 合并残留闭合**: 12 功能 hat,删除 `progress-steward` / `shipper` / fallback `fixer`;新增 `task-planner` 依赖审计 + execution-plan artifact(成功后 emit 恰好一条 `execution.plan.ready` handoff,失败仅 `plan.blocked`)、`exec-wave-dispatcher` 由 `execution.plan.ready` 唤醒后依赖感知迭代 ready waves(不再 trigger `work.ready`)、review-coordinator 7 payload (testing + 6 dims + merge-conflict) SharedReadonly wave、alignment read-only、reporter 单一 owner;rusqlite-backed supervisor wave orchestration, per-slot worktrees, fan-in merge, parallel review/fix; 默认 features 已含 `supervisor-db`,只需运行时 `event_loop.supervisor.enabled: true`), `ce-executor-lite` (template), `debug`, `merge-batch` (Git-first 多 worktree 批量 merge: reviewer → integrator → stabilizer 自环 → reporter), `parallel-forge` (**execution_model: supervisor** — Spec-First/BDD/ATDD/Outside-In 规划(planner 写 wave/order/digest → `forge.plan.ready` 含 `execution_wave` / `integration_order` / `execution_plan_digest` / `wave_total`) → Guardian 并发审批 → Worktree 隔离(forge.wave.worktrees.ready 带 `verified_base_commit`) → forge-dispatcher `ralph wave emit exec.unit.ready` 波次调度 → Executor 槽位 TDD(本 Unit 的 task 在 `forge.wave.settled` 由 CloseTaskBatch 投影原子批量关闭,executor 不可手工 task close) → Reviewer(`forge.wave.reviewed` / `forge.wave.review.failed` 含 failure_fingerprint) → 串行 Integrator 按 `integration_order` FF 整合(`forge.wave.integrated` → `forge.wave.settled`) → Verifier 全量门禁(`forge.wave.verified` / `forge.verification.failed`) → Tester 全量回归(`forge.full.verified` / `forge.full.verification.failed`,3 轮 correction 耗尽 → `forge.final.correction.settled`(schema `allowed_values: correction_round=[3]` runtime 强制,correction_round<3 拒绝)) → Auditor → Reporter 经理报告; 失败走 forge-failure-handler(订阅 `exec.wave.failed` / `forge.wave.review.failed` / `forge.verification.failed`;`exec.wave.failed` 的 `slot_failures[].failure_class` 全为永久类时直发 `forge.plan.blocked`,否则进 3 轮 correction + fail-confidence-rubric 90/75 confidence gate 才允许 `work.failed`)), `post-merge-converge` (一个或多个开发计划已在当前分支/最终代码树中完成后的系统收敛: baseline → change-map → 六维 system-auditor → test-gap → reproducer 自环 → fixer 自环(根因/最小修复/回归) → closer(干净环境+独立终审) → reporter(REPORT.md+report_path 硬门禁); 8 hat 保留 PDF 15 步语义; 置信度 redo-first), `red-team-attack` (实验驱动 Red Team 分析: 从 Git 历史反向定位计划实现提交 → 重建 Patch → 设计并执行真实攻击实验(控制组+攻击组) → 硬阈值证据门禁 → 零回归修复计划; 全程只读代码树,不修改生产代码,最终 PLAN.md 等待人工确认), `merge-loop`(内部单 loop 自动 merge;裸 `ce-executor` 已删除:所有 plan-driven 执行请使用 `ce-executor-pipeline`;仅作模板时可使用 `ce-executor-lite`)
