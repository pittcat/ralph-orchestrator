...
> **⚠️ HARD RULE 1: 测试入口必须用 `cargo nextest run` 系列**(`./scripts/run-tests.sh` / `just test-parallel` / `cargo nextest run -p <pkg> --bin <bin> -- <subset>`)。**禁止**裸跑 `cargo test -p ralph-cli` 或 `cargo test -p ralph-cli --bin ralph`——历史根因是 `crates/ralph-cli/src/loop_runner/tests.rs`(原文件)头部注释的 4 个 process-global Mutex + 时间敏感测试(`std::thread::sleep(500ms)`)。Nextest 的 process-per-test 隔离 Mutex 是第一道保险。**唯一例外**:IDE rust-analyzer/IntelliJ 默认走裸 cargo test 时,务必加 `-- --test-threads=1`,否则 `FAKE_PATH_BACKEND_*` Mutex fixture 串台。其它允许的例外:① `cargo test --doc` 跑 doctest;② nextest 不可用时的最后兜底 `cargo test --workspace --exclude ralph-e2e -- --test-threads=1`;③ `RALPH_BASELINE_SERIAL=1 ./scripts/run-tests.sh` 强制 flake 兜底（走单线程,仅用于竞态/时序 flake 恢复,不是默认路径）;④ `crates/ralph-core/data/ralph-tools*.md` 这类**仅文档用途**的 cargo test 引用。详见 `docs/solutions/developer-experience/ralph-cli-loop-runner-tests-must-run-serial.md`。
>
> **⚠️ HARD RULE 1a: `cargo-nextest` 版本钉死在 `mise.toml`**(SSOT,2026-07-25 起)。字段 `"cargo:cargo-nextest" = "0.9.140"`。**不要**写进 `Cargo.toml`/`Cargo.lock`(那是 crate 依赖,不是 host CLI)。换机器:`mise install`,或无 mise 时 `just nextest-install` / `cargo install cargo-nextest --locked --version 0.9.140`。CI 用 `taiki-e/install-action@nextest` 的 `tool-version: 0.9.140`,必须与 `mise.toml` 同步。**版本检查**:`cargo nextest --version` 应输出 `0.9.140`。Node.js **不**走 mise,用 nvm + `.nvmrc`。
>
...
### Supervisor Scheduler Mode（`event_loop.supervisor.scheduler_mode`）
...
- **DAG typed `JobContext`（plan 2026-09-09-0917 U15 落地后启用）**：DAG 模式下，每个 active job 接收一个 typed `JobContext`，由 runtime 通过 env 注入 hat 子进程；hat 不应再依赖旧的 wave/slot 标识。当前暴露字段：`plan_key` / `unit_key` / `task_key` / `task_id` / `job_id` / `job_token` / `stage` / `attempt` / `worktree` / `base` / `verified_execution_plan_path` / `artifact_refs`。**已弃用/移除**：`wave_id` / `slot_index` / `worktree_map`——hat 不应再读这三个字段（runtime 不再注入，旧 payload 字段被 typed schema 拒收）。
- **SQL migrations v19-v23（DAG store）**：`dag_capacity_reservations`（v19，per-Unit capacity 槽位预留）、`dag_units_completion_ack`（v20，exactly-once close acknowledgement）、`dag_corrected_state_machine`（v21，correction reentry 持久化）、`dag_artifact_refs`（v22，typed artifact_refs 列）、`dag_runtime_virtual_target`（v23，`dag_runtime` 虚拟 target 路由表）已落地；新安装自动迁移，旧库由 `ralph loops clean --ledger` + migration runner 升级。详见 `crates/ralph-cli/src/loop_runner/dag_scheduler/store/migrations/`。
- **可见 topic 族**：DAG 模式下，preset 中 hat subscribers 看到的协调 topic 由 `forge.unit.*` 族（`forge.unit.ready` / `forge.unit.executed` / `forge.unit.failed` / `forge.unit.correction` 等）承担；旧的 `forge.wave.*` 族（`forge.wave.verified` 等）在 `dag` 模式下不再由 hat 订阅（dispatcher 内部 seam 仍保留为过渡期）。hat `instructions` 中涉及协调语义时引用 `forge.unit.*`，不要引用已弃用的 `forge.wave.*`。
...
- Builtin presets: `autoresearch`, `implementation-review` (6-hat wave review→fix-plan, no supervisor), `ce-executor-pipeline` (14+3 hat 线性一条龙 **Ralph primary path**: plan-reviewer → executor(每个 U-ID 一个 subagent,主 executor 负责验收/提交/最终 emit) → `test-stabilizer`(executor 后一次性测试稳定化门禁,可改测试+生产代码,带 correction ID 与全量门禁) → 6 串行维度 hat → review-synthesizer → fix-planner → fixer(修复一轮并跑全量测试) → alignment → reporter; plan 2026-07-29-001 起 `work.failed`/`fix.done`、plan 2026-07-30 起 `work.done` 共挂 3 个合成 precheck gate hat——done/failed 对称:成功路径(`work.done` / `fix.done` applied)走轻量落地核验(核验 completed Units 真有交付 commit + 验证状态诚实,语言无关),失败路径(`work.failed` / `fix.done` partial|blocked)走完整证据审计;证据不足打回 producer,3 次耗尽 runtime 直发 plan.blocked{kind=precheck_exhausted};成功路径另叠加 5 条确定性 payload_consistency 结构矛盾门禁(work-done complete/commit 一致性 + fix-done applied 一致性,emit 时零 LLM 成本)), `ce-executor-pipeline-loop` (`ce-executor-pipeline` 的环形 review/fix 版本: work.done → `test-stabilizer` → review-reentry → 6 串行维度 review → review-gate；fix.done → review-reentry 直接进入下一轮复审；按 `must_fix_now` 收敛门控继续修复,baseline / 范围外 / 新发现残留进入报告,最多 6 轮), `ce-executor-lite` (template), `debug`, `merge-batch` (Git-first 多 worktree 批量 merge: reviewer → integrator → stabilizer 自环 → reporter), `parallel-forge` (**execution_model: supervisor+dag** — planner emits `forge.plan.ready` with the execution plan; Guardian approves concurrency; the active DAG runtime owns Unit admission, durable isolated jobs, per-Unit execution/review/verification, deterministic integration, correction reentry, task-close projection, and exactly-once `forge.exec.development.done`; Auditor and Reporter complete the final delivery path), `post-merge-converge` (一个或多个开发计划已在当前分支/最终代码树中完成后的系统收敛: baseline → change-map → 六维 system-auditor → test-gap → reproducer 自环 → fixer 自环(根因/最小修复/回归) → closer(干净环境+独立终审) → reporter(REPORT.md+report_path 硬门禁); 8 hat 保留 PDF 15 步语义; 置信度 redo-first), `red-team-attack` (实验驱动 Red Team 分析: 从 Git 历史反向定位计划实现提交 → 重建 Patch → 设计并执行真实攻击实验(控制组+攻击组,实验在 `.worktrees/redteam-<RTE-id>/` 独立 git worktree 内可改代码) → 硬阈值证据门禁 → 零回归修复计划; 宿主 loop checkout 只读且只写 `.ralph/red-team/**`,最终 PLAN.md 等待人工确认), `merge-loop`(内部单 loop 自动 merge;裸 `ce-executor` 已删除:所有 plan-driven 执行请使用 `ce-executor-pipeline`;仅作模板时可使用 `ce-executor-lite`)
...
- **Operator skills（preset 起草/评审）:** `skills/ralph-preset-author`、`skills/ralph-preset-review`（各自携带本地 `references/`，见各 skill 目录；不再存在 `skills/ralph-preset-common` 共享目录）
...
- **Plugin-side contracts (e.g. `plugins/nowledge-mem-ralph/README.md` auto-finalization marker spec) evolve independently of the runtime HARD RULES in this file.** Plugin maintainers update README + plugin code under `plugins/knowledge-mem-ralph/**`; the HARD RULES below apply to `ralph-cli` / `ralph-core` only and are not the source of truth for plugin behaviour.
- **Plan 残留文件不进 git(HARD RULE)**: `.ralph/review/<plan-id>/{residuals*.md,scratch/,draft/}` 是 plan 跑完后的过程产物(同 plan_id 下任何子目录都生效),**禁止 commit**。reviewer / executor 完成 plan 后应把有用内容合并到 `docs/solutions/` 或 plan 主文件,然后删除本地文件;无用则直接删除。**.gitignore 已配置兜底规则**(对新文件生效),pre-commit hook 也会硬拦截(对被错误 `git add` 的文件 fail-fast)。**如果 `git status` 显示这些文件被 `D`**:说明之前被误 commit,用 `git rm <file>` 清理工作区 + 索引,**不要** `git rm --cached`(因为本地也不应保留),然后单独 commit `chore: remove tracked plan residuals`。
...
- **AI skill guide 去计划化规则(HARD RULE)**: `crates/ralph-core/data/*.md` 是所有 agent 共用的通用注入 skill，**禁止**写入只适用于某个计划、某次事故、某个特定 preset 或某个临时修复的内容（例如 `plan 2026-*`、`U*` 编号、具体 preset 名、诊断报告路径、过窄样例、reviewer-only 背景）。注入 skill 里只保留可复用的通用规则和抽象约束；如果必须保留实现背景或一次性修复细节，只能放到非注入开发文档或代码注释。
- **AI skill guide 作用域边界(HARD RULE)**: `crates/ralph-core/data/*.md` 只允许写 loop 内 agent 在当前 activation 中需要执行的、可复用的 agent-facing 规则。operator-only 的 CLI 参数与启动流程（例如外层 loop 的 worktree 创建/复用、`--plan` 绑定）、runtime 内部实现细节、以及 preset 专属操作说明，不得写入自动注入或 agent 可见 skill；应放到 CLI `--help`、operator 文档或 preset 自身配置中。条件恢复 skill 只能描述 agent 收到恢复信号后必须执行的动作，不得把 operator 控制面重新注入 agent prompt。
...
- **Preset operator skills 同步规则(HARD RULE)**: `skills/ralph-preset-author`、`skills/ralph-preset-review` 是 **loop 外** preset 起草/AAF 评审的操作规程；每套 skill 各自携带本地 `references/`（author 用 `skills/ralph-preset-author/references/`,review 用 `skills/ralph-preset-review/references/`；`review` 还拥有 `fixtures/` 与 `tests/`），必须随 Ralph 能力演进保持可审计。**在做计划阶段**,若变更影响 preset 作者或评审应知道的能力,必须在计划里写明是否同步这两套 skill。**在完成代码实现后**,必须反向检查并更新,否则 review 会对新违规假阴性、对旧假设假阳性。触发范围包括但不限于:新增/删除/重命名 `ralph preset` / `ralph hats` / `ralph emit` 子命令或参数;`ralph preset check` / `preset_lint` 新增或变更 `finding_id`、severity、strict 语义;`event_loop` / hat / `event_policy` / `state_projection` / isolated prompt 注入行为变更;新增 event topic、`required_fields`、OPAC/单事件预算/三字段约束;AAF 评审所依赖的 CLI 白名单或 hat 可见性模型变更。**允许编辑范围**:`skills/ralph-preset-author/references/{agent-native-model,author-checklist,commands,finding-rubric,patterns,prompt-visibility}.md`(author 视图);`skills/ralph-preset-review/references/{agent-native-model,author-checklist,commands,finding-rubric,patterns,prompt-visibility,agent-skill-audit}.md`(review 视图);`skills/ralph-preset-review/fixtures/`(所有 fixture + README);`skills/ralph-preset-review/tests/test_skill_anchors.py`;`skills/ralph-preset-{author,review}/SKILL.md`(workflow/guardrails)。**最低验收**:更新两份 `finding-rubric.md` 与 `commands.md`(author / review 各持一份)中受影响 `finding_id` 映射与命令表;`commands.md` 与 `ralph <cmd> --help` 一致;对 `skills/ralph-preset-review/fixtures/aaf-review-negative-fixture.yml` 或等价场景重跑 review 流程说明仍成立。`skills/ralph-preset-common` 不再存在;复制 anchor 一致性由 `crates/ralph-core/src/capability_inventory.rs` 的 compile-time include_str + `skill_inventory_yes` 测试保证。
...
- **Preset 测试规则(HARD RULE)**: 不要新增只校验 preset YAML / hat instructions / prompt 文案是否包含某段文本的测试,也不要新增用 byte-equality 锁定 root preset 与 embedded preset 完整内容的测试。Preset 文本和 prompt 内容会随着经验、术语、流程改进持续演进;锁定文本既不能证明行为正确,也会频繁触发维护成本。需要覆盖 preset 约束时,优先测试结构化语义:`RalphConfig::parse_yaml`、hat `triggers` / `publishes`、`event_policy.schemas`、`required_fields`、preset lint findings、BDD scenario 或真实 runtime path。只有当某段文本本身是稳定的用户可见契约(如对外 manifest / index.json 入口的描述字段)时,才允许断言精确文案,并必须在测试注释中说明例外理由。
- **preset yml 改动后必须同步 schema 并跑校验(HARD RULE)**:修改 `presets/en/<name>.yml` 后,必须检查 `presets/schemas/<name>.yml`(SSOT)是否需要同步修改——event schema 字段增删改、`required_fields` 变更、`execution_contracts` 规则调整、`topic_deny_rules` 变更等都要同步。改完必须跑 yml 校验:`cargo nextest run -p ralph-cli --bin ralph -- preset_lint` + `cargo nextest run -p ralph-core -- preset_lint` + `cargo nextest run -p ralph-cli --bin ralph -- presets`(覆盖 `presets_array_matches_manifest` / `test_all_embedded_presets_pass_strict_lint` 等结构化 parity 检查)。schema 与 preset 不一致或漏跑校验视为违规。
- **preset/schema 改动后的下游同步清单(HARD RULE)**:改 `presets/schemas/<name>.yml` 或 `presets/en/<name>.yml` 的 event 拓扑(终态事件增删、hat triggers/publishes、required_fields、topic_deny_rules)后,必须逐层检查并同步以下下游,任一漏改即违规:
  ...
  改完跑全量校验:`./scripts/run-tests.sh`(含 preset_lint + WAC + scenarios + 结构化 schema/preset parity)。
...
- **测试入口强制 nextest(HARD RULE 1 + 2); hat env scrub 见 HARD RULE 5**:
...
- **测试不得被外层 hat env 污染(HARD RULE 5)**:写/改任何会 spawn `ralph` CLI 的测试时,**必须**假设进程可能继承 `ralph run` hat 注入的 agent-context 环境变量(`RALPH_CURRENT_HAT` / `RALPH_CURRENT_LOOP_ID` / `RALPH_EVENTS_FILE` / `RALPH_WAVE_WORKER`,以及 `RALPH_TRIGGERED_HAT` / `RALPH_HATS_SOURCE` / `RALPH_CONFIG`)。
  - **Human-CLI 假设的测试**:spawn 前必须 scrub 上述键。`ralph-cli` 集成测试**禁止**再写裸 `Command::new(env!("CARGO_BIN_EXE_ralph"))`;一律用 `crates/ralph-cli/tests/common/mod.rs` 的 `common::ralph_bin()`(或先 `scrub_agent_runtime_env`)。漏 scrub 视为回归:人壳过、pipeline hat 内挂。
...
- **Worktree 复用规则(HARD RULE 3)**:任何使用 `--worktree` 的 `ralph run` 都必须显式指定复用键:**`--plan <plan.md>`** 或 **`--worktree-name <name>`**。Ralph 按 plan basename(去掉 `.md`/`.html` 后缀)或精确名称绑定 worktree:已有同名已完成 worktree 时,先把旧 runtime 记录归档到该 worktree 的 `.ralph/reuse-history/` 再复用;第一次不存在时按同一精确名称创建,不追加随机后缀。正在运行的同名 worktree 不可复用。复用 `parallel-forge` 的 worktree 时,cleanup 前 runtime 会先记录恢复边界;下次启动身份(plan / preset / 配置 / worktree 名称)校验通过则 loop 经 `task.resume` 恢复通道从 pending hat 继续而非重启流程;身份不一致时复用在 loop 启动前被拒绝,错误信息指向该 worktree `.ralph/reuse-history/` 下的归档记录。严禁 agent 自行 `EnterWorktree` / `git worktree add`,也禁止靠 prompt 文本"猜测"plan 路径。旧版"从 prompt 文本自动提取 plan 路径做模糊匹配"的行为已废弃,因为它对中文、标点或附加说明极其脆弱。推荐写法:
  ```bash
...
- **Hat `instructions:` 必须用 hat 视角编写(HARD RULE 4)**:**preset 中每个 hat 的 `instructions:` 字段是写给「该 hat 在自己那一轮 activation 里的 agent」看的**,不是给 preset 作者看的,也不是给该 hat 能看到的其它 hat 看的。**`event_loop.execution_mode: isolated` 是 preset 的默认且唯一的执行模式**(2026-06-18 起全 preset 收敛到 isolated;coordinator 模式已弃用),在此模式下:
  ...
  6. **违规示例(已发现并修复)**:2026-07-03 一个 supervisor-enabled builtin preset(`ce-executor-supervisor`,已于 plan 2026-08-09-001 删除)的 `coordinator` hat 第一版 instructions 写"read `.ralph/events.jsonl` tail"——coordinator 看不到 events.jsonl,因为 events.jsonl 是 supervisor 内部 ledger,只有 supervisor / runtime 进程能写。正确写法是"用 `ralph tools task list -s done` 查 unit 状态"。
  ...
  11. **终态 reporter 必须 artifact-first**:`ralph events --events-source main` 在 reporter activation 中只用于诊断、审计和 wave 写盘确认,不得作为报告业务输入。报告状态必须来自当前 trigger payload;复杂聚合由上游写入版本化、可校验的 repo-relative artifact,再把路径随 trigger 传递。reporter 只能读取 trigger 指向的业务 artifact,不得扫描 main history 重建执行、评审、修复或对齐事实。
