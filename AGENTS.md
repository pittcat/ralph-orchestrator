...
- **Worktree 复用规则(HARD RULE 3)**:执行任何 dev plan、或调用任何带 `--worktree` / `EnterWorktree` / `git worktree add` 的入口前,**必须先跑 `git worktree list`**(不能跳过、不能凭印象判断),并对结果做**模糊匹配**——只要 worktree 路径或分支名中包含本 plan 的 basename(如 `2026-06-25-002-feat-profiles-for-preset-role-tuning-plan`)就算匹配。命名约定:`<plan-basename>-<adjective-noun>`(plan 文件去掉 `.md` 后缀 + 两个随机英文单词,例如 `2026-06-25-001-feat-ce-executor-serial-5dim-coordinator-amendments-plan-lucky-reed`),模糊匹配允许后缀随机词不同。**有匹配则直接 `cd` 进去复用,严禁创建**;只有确认无匹配时,才按上述约定创建新 worktree(避免重名/重复消耗磁盘,也避免与已 in-flight 的 loop 状态错位)。
