...
- Builtin presets: `autoresearch`, `ce-executor-pipeline` (13-hat 线性一条龙 **Ralph primary path**: plan-reviewer → executor(每个 U-ID 一个 subagent,主 executor 负责验收/提交/最终 emit) → 6 串行维度 hat → review-synthesizer → fix-planner → fixer → alignment → reporter), `ce-executor-pipeline-loop` (`ce-executor-pipeline` 的环形 review/fix 版本: work.done/fix.done → review-reentry → 6 串行维度 review → review-gate；按 `must_fix_now` 收敛门控继续修复,baseline / 范围外 / 新发现残留进入报告,最多 6 轮), `ce-executor-supervisor` (2026-07-03-001 plan: 16-hat; rusqlite-backed supervisor wave orchestration, per-slot worktrees, fan-in merge, parallel review/fix; 需用 `--features supervisor-db` 编译且 `event_loop.supervisor.enabled: true`), `ce-executor-lite` (template), `debug`, `merge-batch` (Git-first 多 worktree 批量 merge: reviewer → integrator → stabilizer 自环 → reporter), `merge-loop`(内部单 loop 自动 merge;裸 `ce-executor` 已删除:所有 plan-driven 执行请使用 `ce-executor-pipeline`;仅作模板时可使用 `ce-executor-lite`)
