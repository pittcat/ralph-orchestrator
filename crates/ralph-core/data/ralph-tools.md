...
8. **`## RECEIVER CONTRACT` 区块列出你本 hat 的 emit 契约** —— 当 prompt 中出现 `## RECEIVER CONTRACT` 时，它由 runtime 根据当前 hat 声明的可发 topic 列表与 preset 的 event schema 自动生成，逐条列出你可以 emit 的 topic 及其必填字段；构造 payload 时以该清单为准（可用 `ralph emit --schema <TOPIC>` 复核字段定义），某个 topic 没有附带字段清单时表示 preset 未为它声明 schema 约束。若无法确认必填字段或某个必填值未知，停止 emit，报告缺少的信息并等待有效输入或恢复信号。

## 收到 `task.resume` 时（policy / origin / contract 拒收后自动注入）

收到 `task.resume` 后，先读取恢复 payload 和 `## CORRECTION CONTEXT`（如果存在），再按其中的 `reason`、`kind`、`required_fields`、`original_trigger_topic`、`original_trigger_payload`、`allowed_topics` 等实际字段确认下一步动作。`target_hat` 如果存在，只用于核对恢复上下文；旧 payload 可能没有该字段，不要把它当成必填字段。

如果上下文明确要求重新发布业务事件，先用 `ralph emit <topic> --policy-check` 预检，再按 `ralph-tools-emit` 的 Confirm 规则正式发布。不要猜测缺失字段、修改 preset 路由、重新广播或原样重发同一恢复 payload。

如果无法确认恢复对象、原始触发上下文或合法发布范围，停止本次恢复动作；用 `ralph inspect loop --format json` 或 `ralph tools task list` 复核状态。复核后仍无法确定时，报告阻塞原因并等待后续恢复信号。
...
8. **缺少可用上下文时停止**：如果必须使用的字段为 `<missing>`，或字段之间互相矛盾，不要自己重新广播或重发同一 payload；读 `ralph inspect loop --format json` / `ralph tools task list` 复核当前状态。仍无法确认时停止并报告阻塞原因。

## Activation exit protocol（激活退出协议）

每个 hat 的终态事件（terminal topic）由当前 preset 的 `event_policy.schemas` 声明。退出协议如下：

1. **确定终态 topic**：当前 hat 的 `publishes` 中存在 `terminal_topics`；若有 `primary_terminal_topic`（单条）则直接用，否则从 `terminal_topics` 中按 preset 拓扑选一条。
