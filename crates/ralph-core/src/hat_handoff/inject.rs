...
pub fn build_block(
    ...
    // R21 hard validation (plan 2026-06-20-001). The artifact
    // rule comes from the embedded `hat_handoff.artifact` SSOT
    // section after `build.rs` merge; failing the rule means the
    // artifact does not match the protocol's required shape and
    // must be regenerated by the agent. Returning `None` here is
    // fail-closed — the inject point in `event_loop/mod.rs`
    // records a diagnostic and skips the block.
    config.artifact.validate(&content).ok()?;
