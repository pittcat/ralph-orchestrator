...
impl WorkspaceMutationDelta {
    pub fn is_clean(&self) -> bool {
        !self.head_changed
            ...
            && self.input_artifact_changed.values().all(|changed| !*changed)
    }
}
...
pub fn validate_allowed_path(rule: &str) -> Result<(), PathError> {
    if rule.is_empty() { return Err(PathError::Empty); }
    if rule.starts_with('/') { return Err(PathError::Absolute); }
    if rule.contains('\\') { return Err(PathError::Backslash); }
    if rule.starts_with(".git/") || rule == ".git" || rule.starts_with(".git/**") {
        return Err(PathError::DotGitPrefix);
    }
    for seg in rule.split('/') {
        if seg == "." || seg == ".." { return Err(PathError::DotSegment); }
    }
...
pub fn validate_plan_key(plan_key: &str) -> Result<(), PathError> {
    if plan_key.is_empty() { return Err(PathError::UnsafePlanKey(plan_key.to_string())); }
    if plan_key.contains('/') || plan_key.contains('\\') { return Err(PathError::UnsafePlanKey(plan_key.to_string())); }
    if plan_key == "." || plan_key == ".." { return Err(PathError::UnsafePlanKey(plan_key.to_string())); }
    if plan_key.starts_with('.') { return Err(PathError::UnsafePlanKey(plan_key.to_string())); }
    Ok(())
}
...
pub fn path_allowed_by(path: &str, expanded_rules: &[String]) -> bool {
    for rule in expanded_rules {
        if let Some(prefix) = rule.strip_suffix("/**") {
            ...
            if path == prefix || path.starts_with(&format!("{prefix}/")) { return true; }
        } else if path == rule {
            return true;
        }
    }
...
pub fn capture_snapshot(
    ...
    fn git_capture(args: &[&str], cwd: &Path) -> Result<std::vec::Vec<u8>, CaptureError> {
        let out = std::process::Command::new("git").args(args).current_dir(cwd).output()
            .map_err(|e| CaptureError::Git { command: args.join(" "), stderr: e.to_string() })?;
    ...
    let index_bytes = git_capture(&["diff", "--cached", "--raw", "-z", "--no-renames", "HEAD", "--"], workspace_root)?;
    let index_digest = sha256_hex(&index_bytes);

    // 4. tracked digest (raw NUL)
    let tracked_bytes = git_capture(&["diff", "--raw", "-z", "--no-renames", "--"], workspace_root)?;
    let tracked_digest = sha256_hex(&tracked_bytes);

    // 5. untracked paths (NUL-separated), filtered by allowed roots
    let untracked_raw = git_capture(&["ls-files", "--others", "--exclude-standard", "-z", "--"], workspace_root)?;
    let mut untracked_paths: BTreeSet<String> = BTreeSet::new();
    for entry in untracked_raw.split(|b| *b == 0) {
        if entry.is_empty() { continue; }
        let s = std::str::from_utf8(entry).map_err(|_| CaptureError::Git {
    ...
    for op in ["MERGE_HEAD", "CHERRY_PICK_HEAD", "REVERT_HEAD", "BISECT_LOG"] {
        ...
        let abs = if Path::new(&rel).is_absolute() { PathBuf::from(&rel) } else { workspace_root.join(&rel) };
    ...
    if let Some(payload) = trigger_payload.and_then(|v| v.as_object()) {
        for (key, val) in payload {
            ...
            let abs = if Path::new(p).is_absolute() { PathBuf::from(p) } else { workspace_root.join(p) };
            let meta = std::fs::metadata(&abs).map_err(|_| CaptureError::InvalidInputArtifact(p.into()))?;
            if !meta.is_file() { return Err(CaptureError::InvalidInputArtifact(p.into())); }
            let bytes = std::fs::read(&abs).map_err(|_| CaptureError::InvalidInputArtifact(p.into()))?;
...
pub fn compute_delta(
    ...
    let mut untracked_added: Vec<String> = after.untracked_paths.difference(&before.untracked_paths).cloned().collect();
    untracked_added.sort();
    let mut untracked_removed: Vec<String> = before.untracked_paths.difference(&after.untracked_paths).cloned().collect();
    ...
    WorkspaceMutationDelta {
        head_changed, ref_changed, index_changed, tracked_changed,
        untracked_added, untracked_removed, git_op_changed, input_artifact_changed,
    }
}
...
mod tests {
    ...
    fn validate_allowed_path_rejects_illegal() {
        ...
        assert!(matches!(validate_allowed_path("/abs"), Err(PathError::Absolute)));
        assert!(matches!(validate_allowed_path("a\\b"), Err(PathError::Backslash)));
        assert!(matches!(validate_allowed_path("a/./b"), Err(PathError::DotSegment)));
        assert!(matches!(validate_allowed_path("a/../b"), Err(PathError::DotSegment)));
        assert!(matches!(validate_allowed_path(".git/HEAD"), Err(PathError::DotGitPrefix)));
        assert!(matches!(validate_allowed_path(".git/**"), Err(PathError::DotGitPrefix)));
        assert!(matches!(validate_allowed_path("reviews/**/nested"), Err(PathError::NonTrailingStarStar(_))));
        assert!(matches!(validate_allowed_path("reviews/**.bak"), Err(PathError::NonTrailingStarStar(_))));
        assert!(matches!(validate_allowed_path("a/{other}/b"), Err(PathError::UnknownVariable(_))));
    }
    ...
    fn path_allowed_by_matches_exact_and_dir_prefix() {
        ...
        assert!(!path_allowed_by("reviews-old/a.md", &rules), "no string-prefix leak");
    }

    #[test]
    fn expand_plan_key_substitutes_only_plan_key_variable() {
        assert_eq!(expand_plan_key("reviews/**", "demo"), "reviews/**");
        assert_eq!(expand_plan_key(".ralph/forge/{plan_key}/reviews/**", "demo"),
    ...
    fn delta_detects_head_change() {
        let a = WorkspaceMutationSnapshot { head: "abc".into(), symbolic_ref: None, index_digest: "x".into(), tracked_digest: "y".into(), untracked_paths: BTreeSet::new(), git_op_sentinels: BTreeMap::new(), input_artifact_digests: BTreeMap::new() };
        let b = WorkspaceMutationSnapshot { head: "def".into(), ..a.clone() };
    ...
    fn delta_detects_tracked_and_untracked_and_git_op_changes() {
        let a = WorkspaceMutationSnapshot {
            head: "abc".into(), symbolic_ref: None, index_digest: "x".into(), tracked_digest: "y".into(),
            untracked_paths: BTreeSet::new(),
        ...
        let b = WorkspaceMutationSnapshot {
            ...
            git_op_sentinels: BTreeMap::from([("MERGE_HEAD", "h2".to_string()), ("REVERT_HEAD", "h3".to_string())]),
            ..a.clone()
        };
        let d = compute_delta(&a, &b, &[]);
    ...
    fn delta_detects_input_artifact_digest_change() {
        let a = WorkspaceMutationSnapshot {
            head: "abc".into(), symbolic_ref: None, index_digest: "x".into(), tracked_digest: "y".into(),
            untracked_paths: BTreeSet::new(), git_op_sentinels: BTreeMap::new(),
            input_artifact_digests: BTreeMap::from([("readonly_evidence_path".into(), "d1".into())]),
        };
        let b = WorkspaceMutationSnapshot {
            input_artifact_digests: BTreeMap::from([("readonly_evidence_path".into(), "d2".into())]),
            ..a.clone()
        };
        let d = compute_delta(&a, &b, &[]);
        assert_eq!(d.input_artifact_changed.get("readonly_evidence_path"), Some(&true));
    }
}
