...
// U10 (plan 2026-08-05-001): re-export prompt surface types
// from `prompt_types` so the `event_loop::prompt::*` re-export
// path (sibling `prompt.rs`) keeps resolving without a
// detour via the new module name. Stable API preserved.
// Also re-exports `OverEmitRecovery` and `RecoverableExhaustion`
// because `loop_state.rs` references them via fully-qualified
// `crate::event_loop::*` paths. `PromptSkillEntry` and
// `PromptSkillSource` are re-exported because in-tree tests
// that use `use super::*` rely on the namespace wildcard.
pub use prompt_types::{
    OverEmitRecovery, PromptPreview, PromptSkillEntry, PromptSkillSource, RecoverableExhaustion,
    SkillGateFlags, default_evidence_level,
};
...
// U10 (plan 2026-08-05-001): inline `use` imports for items
// moved out of this file into `prompt_types` and `flow_wiring`.
// The whole point of the split is to shrink the root file
// without changing the public API or call sites — so the bare
// names that `impl EventLoop` already uses keep working.
use self::flow_wiring::{
    build_invalid_step_target_resume_payload_for_jsonl, build_stage_pipeline_from_config,
    effective_mechanism_config, preview_prompt_for_config, unknown_fix_step,
};
use self::prompt_types::{
    is_commit_first_business_topic, is_isolated_exempt_topic, strip_human_guidance_block,
    SkillInjector,
};
...
// Inline test mods extracted to sibling files.
