...
fn u6_fix_applied_contract_present_in_ce_executor_serial_preset() {
    // Static preset-level assertion: the `ce-executor-serial`
    // preset MUST declare `fix.applied` with `commit_only`
    // git-evidence mode AND `commit_count` as a required
    // payload field. Without this the perky-maple P2-3 drift
    // path re-opens.
    //
    // 2026-06-20-001 (plan F-PS-005 + KTD-12): `require_payload_fields`
    // is no longer declared inline; the canonical required-field
    // set lives in `event_policy.schemas.fix.applied.required_fields`
    // (the SSOT the engine's `effective_required_fields` joins
    // against). This test now reads the **merged** output the
    // `build.rs` produces (the same YAML the engine sees at
    // runtime) so we exercise the post-merge contract rather
    // than the authoring file's residual inline layers.
    //
    // The test deliberately walks `target/.../ce-executor-serial.yml`
    // — `build.rs` writes the merged file alongside the embedded
    // preset; if `cargo build` has not been re-run since the SSOT
    // was updated, the merged file may be stale, so the test
    // loads the SSOT directly and applies the same merge in
    // memory. Using the SSOT directly is more portable than
    // depending on `target/...` path stability across machines.
    use crate::config::RalphConfig;
    use std::path::Path;

    // Load the SSOT (schemas file). The schemas file is the
    // KTD-12 source of truth for `required_fields` — it lists
    ...
    assert!(
        ...
        "fix.applied schema MUST require commit_count, got {:?}",
        fix_applied_required
    );
    assert!(
        fix_applied_required.contains(&"fix_round".to_string()),
        "fix.applied schema MUST require fix_round, got {:?}",
        fix_applied_required
    );

    // The `commit_only` git-evidence mode is still inline
    // (it is part of `execution_contracts.rules.fix.applied`,
