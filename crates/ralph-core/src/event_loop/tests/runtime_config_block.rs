...
//! runtime-resolved `event_loop.*` values (e.g. `max_fix_rounds`,
//! `max_residuals`) to the hat prompt via a `## RUNTIME CONFIG`
//! block.

#[test]
fn append_runtime_config_block_includes_max_fix_rounds() {
    use crate::event_loop::append_runtime_config_block;
    let prompt = append_runtime_config_block("base".to_string(), 1, 8);
    assert!(
...
fn append_runtime_config_block_reflects_custom_value() {
    use crate::event_loop::append_runtime_config_block;
    let prompt = append_runtime_config_block("base".to_string(), 7, 5);
...
fn append_runtime_config_block_preserves_existing_prompt() {
    use crate::event_loop::append_runtime_config_block;
    let prompt = append_runtime_config_block("hello world".to_string(), 3, 8);
