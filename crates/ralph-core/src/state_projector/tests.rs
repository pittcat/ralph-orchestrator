...
// ─────────────────────────────────────────────────────────────────
// U9 of plan 2026-07-05-005: ProjectedField version-protocol
// unit tests. These pin the LangGraph-style optimistic-
// concurrency helper before any caller (U11+) starts wiring
// it into the projector apply path.
// ─────────────────────────────────────────────────────────────────

#[test]
fn u9_projected_field_initial_version_is_zero() {
    let field: ProjectedField<String> = ProjectedField::default();
    assert_eq!(field.version, 0);
    assert!(field.last_writer.is_none());
    assert_eq!(field.get(), &String::default());
}

#[test]
fn u9_projected_field_try_write_none_always_succeeds_and_bumps() {
    let mut field: ProjectedField<String> = ProjectedField::default();
    field.try_write("v1".to_string(), None, Some("hat-A")).unwrap();
    assert_eq!(field.get(), "v1");
    assert_eq!(field.version, 1);
    assert_eq!(field.last_writer.as_deref(), Some("hat-A"));
    field.try_write("v2".to_string(), None, Some("hat-B")).unwrap();
    assert_eq!(field.version, 2);
    assert_eq!(field.last_writer.as_deref(), Some("hat-B"));
}

#[test]
fn u9_projected_field_try_write_expected_match_succeeds() {
    let mut field: ProjectedField<String> = ProjectedField::default();
    field.try_write("v0".to_string(), None, None).unwrap();
    // Caller observed version 1 and asserts it.
    field.try_write("v1".to_string(), Some(1), Some("hat-A")).unwrap();
    assert_eq!(field.version, 2);
}

#[test]
fn u9_projected_field_try_write_stale_version_rejected() {
    let mut field: ProjectedField<String> = ProjectedField::default();
    field.try_write("v0".to_string(), None, None).unwrap();
    field.try_write("v1".to_string(), None, None).unwrap();
    // Caller observed version 1 but actual is 2 → reject.
    let err = field
        .try_write("stale".to_string(), Some(1), Some("hat-stale"))
        .unwrap_err();
    assert_eq!(err.actual, 2);
    assert_eq!(err.expected, 1);
    // Field is unchanged after rejection.
    assert_eq!(field.version, 2);
    assert_eq!(field.get(), "v1");
}

#[test]
fn u9_projected_field_concurrent_writer_second_rejected() {
    // Two hats observed version 0 simultaneously. The first
    // succeeds; the second's write must be rejected.
    let mut field: ProjectedField<String> = ProjectedField::default();
    field.try_write("v0".to_string(), None, None).unwrap();
    field.try_write("first".to_string(), Some(1), Some("hat-A")).unwrap();
    // hat-B reads the same version 1 and tries to write.
    let err = field
        .try_write("second".to_string(), Some(1), Some("hat-B"))
        .unwrap_err();
    assert_eq!(err.actual, 2);
}
