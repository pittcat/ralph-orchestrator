...
/// 新运行必须重放 `.ralph/current-events` 选中的 ledger，而不是历史的
/// `.ralph/events.jsonl` 镜像；否则上一轮 red-team queue 会让新运行的第一个
/// `attack.mapped` 被误判为重复初始化。
#[test]
fn test_builtin_redteam_attack_mapped_uses_current_events_scope() {
    let temp_dir = TempDir::new().expect("temp dir");
    let temp_path = temp_dir.path();
    let ralph_dir = temp_path.join(".ralph");
    std::fs::create_dir_all(ralph_dir.join("red-team")).unwrap();

    let historical_events = ralph_dir.join("events.jsonl");
    std::fs::write(
        &historical_events,
        r#"{"topic":"redteam.attack.mapped","payload":{"experiment_count":18}}
"#,
    ...
    let current_events = ralph_dir.join("events-20260816-141014.jsonl");
    std::fs::write(
        &current_events,
        r#"{"topic":"redteam.plan.resolved","payload":{"scope_status":"resolved"}}
"#,
    )
    .unwrap();
    std::fs::write(
        ralph_dir.join("current-events"),
        format!("{}\n", current_events.display()),
    )
    .unwrap();
    std::fs::write(ralph_dir.join("current-loop-id"), "primary-new").unwrap();
    std::fs::write(
        ralph_dir.join("red-team/04-attack-surface.md"),
        "# Attack surface\n",
    )
    .unwrap();
    std::fs::write(
        ralph_dir.join("red-team/05-experiment-plan.md"),
        "# Experiment plan\n",
    )
    .unwrap();
    ...
    let payload = serde_json::json!({
        "surface_count": 1,
        "experiment_count": 1,
        "attack_surface_path": ".ralph/red-team/04-attack-surface.md",
        "experiment_plan_path": ".ralph/red-team/05-experiment-plan.md",
    ...
    let output = common::ralph_bin()
        ...
        .current_dir(temp_path)
        .env("RALPH_HATS_SOURCE", "builtin:red-team-attack")
        .env("RALPH_CURRENT_HAT", "attack-surface-mapper")
        .env("RALPH_CURRENT_LOOP_ID", "primary-new")
        .env("RALPH_EVENTS_FILE", &current_events)
        .output()
        .expect("execute redteam attack.mapped policy-check");

    assert!(
        output.status.success(),
        "new current-events ledger must not inherit a historical queue: stdout={} stderr={}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    assert!(
        !String::from_utf8_lossy(&output.stdout).contains("redteam_experiment_queue_consistency"),
        "historical attack.mapped must not reject the new run: stdout={}",
        String::from_utf8_lossy(&output.stdout)
    );
    ...
    std::fs::write(
        ...
        concat!(
            r#"{"topic":"redteam.plan.resolved","payload":{"scope_status":"resolved"}}
"#,
            r#"{"topic":"redteam.attack.mapped","payload":{"experiment_count":1}}
"#,
        ),
    )
    .unwrap();
    let duplicate = common::ralph_bin()
        .args([
            "-H",
            "builtin:red-team-attack",
            "emit",
        ...
        .env("RALPH_EVENTS_FILE", &current_events)
        .output()
        .expect("execute duplicate redteam attack.mapped policy-check");
    ...
    assert!(
        ...
        "same current-events ledger must reject duplicate attack.mapped: {duplicate_output}"
    );
    assert!(
        duplicate_output.contains("redteam_experiment_queue_consistency"),
        "duplicate rejection must identify the queue consistency gate: {duplicate_output}"
    );
}
