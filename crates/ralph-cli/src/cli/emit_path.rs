...
pub(crate) fn resolve_emit_path(
    ...
    // 2026-07-27-003 plan U2 (KTD-1): when the dispatcher
    // handshake signals are present (`wave_id` + `slot_index` +
    // `loop_id`), an explicit wave-channel target is validated
    // against the per-wave JSON registry FIRST and the function
    // returns immediately on success. The legacy
    // `.ralph/current-wave-channels` marker is gone, so the basic
    // allowlist (current-events / current-candidate-events /
    // current-hat-events) never lists dispatcher-signed channels;
    // without the early-return the second-stage allowlist loop
    // would reject the bound channel as "not in this loop's
    // events allowlist" and undo the dispatcher signature.
    if let (Some(explicit_target), Some(wid), Some(idx), Some(lid)) =
        (explicit.as_ref(), wave_id, slot_index, loop_id)
    {
