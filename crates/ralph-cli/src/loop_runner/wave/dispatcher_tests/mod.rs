...
//! (plan `2026-08-07-008`). This file is mounted as
//! `loop_runner::wave::dispatcher::tests` via `#[path]` in
//! `dispatcher/mod.rs`, restoring the historical module path so
//! existing `cargo nextest` filters keep matching every test. The
//! file's top level is the body of that `tests` module (no inner
//! `mod tests` wrapper).
...
async fn u4_c1_global_deadline_preempts_wave() {
    ...
    match outcome {
        ...
        other => {
            panic!("expected GlobalDeadlineExceeded (runner-supplied 10s budget), got {other:?}")
        }
    }
}
...
fn helper_matches_inline_expression() {
    ...
    let inline_expression = |wave: &ralph_core::DetectedWave,
                             bridge: &dyn SupervisorBridge|
     -> u64 {
...
fn regression_elapsed_past_deadline_still_times_out() {
    use ralph_core::supervisor::WaveDeliveryState;
    use ralph_core::supervisor::{FailedReason, PhaseInputs, WaveKind, WavePhase, evaluate_phase};
...
fn review_wave_failed_attribution_routes_to_finalizer() {
    ...
    let line = std::io::BufReader::new(std::fs::File::open(&main).expect("events file written"))
        .lines()
        .next()
...
fn aggregate_floor_keeps_partial_at_or_after_work_budget() {
    for (timeout, events, concurrency, budget) in [
        ...
        let floor =
            aggregate_floor_for_attempts(Duration::from_secs(timeout), events, concurrency, budget);
        assert!(
...
fn attempt_aware_aggregate_timeout_uses_effective_cap() {
    ...
    let declared_concurrency =
        attempt_aware_aggregate_timeout(configured, wave_timeout, events_count, 4, retry_budget);
    let effective_cap =
        attempt_aware_aggregate_timeout(configured, wave_timeout, events_count, 1, retry_budget);
...
fn u3_classify_slot_result_non_timeout_err_keeps_dynamic_verbatim() {
    ...
    match classified {
        ...
        other => {
            panic!("expected Failed{{reason=REASON_WORKER_CANCELLED}} + Dynamic(_), got {other:?}")
        }
    }
}
...
fn u5_write_wave_diagnostics_json_writes_correct_file() {
    ...
    let read_back: serde_json::Value = serde_json::from_slice(&bytes).expect("must be valid JSON");
...
fn u5_injected_failed_writes_diagnostics_json() {
    ...
    let write_result = write_wave_diagnostics_json(temp_root.path(), "w-u5-integration", &payload);
    assert!(
        write_result.is_ok(),
        "write must succeed: {:?}",
        write_result.err()
...
fn u3_build_review_done_hints_is_bounded_and_evidence_gated() {
    ...
    store
        .record_slot_terminal_evidence(
            &store_wave_id,
            0,
            &TerminalEvidence::from_event("review.unit.done", "{\"dimension\":\"performance\"}"),
        )
        .unwrap();
...
fn accident_05_merge_completed_review_slots_to_main_writes_and_marks() {
    ...
    let main_contents = std::fs::read_to_string(&main_events_file).expect("main must be written");
