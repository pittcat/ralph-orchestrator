...
pub(crate) fn run_supervisor_fan_in(
    ...
    let action_outcome = match action {
        ...
        ralph_core::supervisor::CoordinatorAction::InjectedFailed {
            ...
            // 2026-07-26-003 plan U4/U5 (KTD5/KTD7): the
            // Review arm takes `Option<&ReviewDoneHints>` so the
            // fan-in can supply main-backscan / store-Completed
            // dimensions that already prove a `review.unit.done`
            // happened in a sibling scope. The U5 integration
            // wires this to a real tail-scan + bridge snapshot;
            // today we pass `None` and the function still emits
            // a payload that is correct for the in-results scope
            // (and, after U5, correct across all three sources).
...
// ─────────────────────────────────────────────────────────────────
// 2026-07-26-003 plan U5 (KTD7 / R4): Review-fail-before-merge
// helper.
// ─────────────────────────────────────────────────────────────────
