...
fn recover_from_workspace_state(
    config: &ralph_core::config::RalphConfig,
    workspace_root: &Path,
) -> String {
    use ralph_core::event_loop::recover_current_plan_step;
...
/// 2026-07-30 fix: `forge.plan.ready` payload must mirror the on-disk
/// `execution-plan.yml` SSOT. Planner has historically hand-constructed
/// the payload and shifted `execution_wave` by -1 (treating the plan
/// doc's "Wave 0..N" narrative label as a 0-based index), which then
/// gets rejected at projection time by
/// `state_projector::task::validate_wave_schedule` Rule 1 with no
/// recovery path (guidance topic was removed by plan 2026-06-28-005).
///
/// This CLI-side precheck closes that gap before the event ever reaches
/// JSONL: parse the payload's `execution_plan_path` from disk, walk its
/// `units[]`, and require (a) the declared `execution_plan_digest`
/// matches the disk value and (b) every `unit_tasks[]` entry's
/// `execution_wave` / `integration_order` matches the disk entry by
/// `task_key`. Mismatches produce a `ValidationError` with
/// `reason_code = "disk_payload_inconsistency"` and a pointer to the
/// first offending unit.
///
/// Other topics: no-op (return `Ok(())`). The check is best-effort:
/// if the disk file is missing, unreadable, or not valid YAML, the
...
pub fn check_forge_plan_ready_disk_consistency(
    ...
    // 1. Parse payload (need execution_plan_path + declared digest +
    //    unit_tasks[]). Non-JSON payloads are already caught upstream
    //    by validate_event; pass them through here.
    let payload: serde_json::Value = match serde_json::from_str(payload_str) {
        Ok(v) => v,
        Err(_) => return Ok(()),
    };

    let plan_path_str = payload
        .get("execution_plan_path")
        .and_then(|v| v.as_str())
        .unwrap_or("");
    if plan_path_str.is_empty() {
        return Ok(());
    }
    let plan_path = std::path::Path::new(plan_path_str);
    let absolute_plan_path = if plan_path.is_absolute() {
        plan_path.to_path_buf()
    } else {
        workspace_root.join(plan_path)
    };

    // 2. Read & parse the disk YAML. Silent-bypass on any IO/YAML
    //    error: this precheck is defence-in-depth, not the primary
    //    gate. Projection still rejects malformed events.
    let disk_text = match std::fs::read_to_string(&absolute_plan_path) {
        Ok(s) => s,
        Err(_) => return Ok(()),
    };
    let disk_value: serde_yaml::Value = match serde_yaml::from_str(&disk_text) {
        Ok(v) => v,
        Err(_) => return Ok(()),
    };

    // 3. Compare declared digest vs disk digest.
    let payload_digest = payload
        .get("execution_plan_digest")
        .and_then(|v| v.as_str())
        .unwrap_or("");
    let disk_digest = disk_value
        .get("execution_plan_digest")
        .and_then(|v| v.as_str())
        .unwrap_or("");
    if !payload_digest.is_empty() && !disk_digest.is_empty() && payload_digest != disk_digest {
        return Err(ValidationError {
            ...
            reason_code: "disk_payload_inconsistency".to_string(),
            message: format!(
                "forge.plan.ready execution_plan_digest='{}' does not match disk '{}' \
                 (execution_plan_path='{}'); the planner must derive the digest from the on-disk \
                 execution-plan.yml instead of computing an independent hash",
                payload_digest, disk_digest, plan_path_str
            ),
            ..Default::default()
        });
    }

    // 4. Compare per-unit wave / integration_order by task_key.
    let disk_units = match disk_value.get("units").and_then(|v| v.as_sequence()) {
        Some(s) => s,
        None => return Ok(()),
    };
    let disk_by_key: std::collections::HashMap<String, (u64, u64)> = disk_units
        .iter()
        .filter_map(|u| {
            let key = u.get("task_key")?.as_str()?.to_string();
            let wave = u.get("execution_wave")?.as_u64()?;
            let order = u.get("integration_order")?.as_u64()?;
            Some((key, (wave, order)))
        })
        .collect();

    let payload_units = match payload.get("unit_tasks").and_then(|v| v.as_array()) {
        Some(s) => s,
        None => return Ok(()),
    };

    for (idx, unit) in payload_units.iter().enumerate() {
        let key = match unit.get("task_key").and_then(|v| v.as_str()) {
            Some(k) => k,
            None => continue,
        };
        let payload_wave = match unit.get("execution_wave").and_then(|v| v.as_u64()) {
            Some(w) => w,
            None => continue,
        };
        let payload_order = match unit.get("integration_order").and_then(|v| v.as_u64()) {
            Some(o) => o,
            None => continue,
        };
        if let Some(&(disk_wave, disk_order)) = disk_by_key.get(key) {
            // Rule: execution_wave must be a positive integer (matches
            // state_projector::task::validate_wave_schedule Rule 1).
            // Catching the violation here short-circuits the silent
            // dead-lock the original 2026-07-30 primary run hit.
            if payload_wave == 0 {
                return Err(ValidationError {
                    payload_index: idx,
                    field: format!("unit_tasks[{idx}].execution_wave"),
                    reason_code: "disk_payload_inconsistency".to_string(),
                    message: format!(
                        "forge.plan.ready unit '{key}' has execution_wave=0 in payload but \
                         disk execution-plan.yml has execution_wave={disk_wave}; the planner \
                         must shift the plan-narrative 'Wave N' label to execution_wave=N+1 \
                         (the runtime requires positive integers per \
                          state_projector::task::validate_wave_schedule Rule 1)"
                    ),
                    ..Default::default()
                });
            }
            if payload_wave != disk_wave || payload_order != disk_order {
                return Err(ValidationError {
                    payload_index: idx,
                    field: format!("unit_tasks[{idx}]"),
                    reason_code: "disk_payload_inconsistency".to_string(),
                    message: format!(
                        "forge.plan.ready unit '{key}' mismatch: payload \
                         execution_wave={payload_wave} integration_order={payload_order} vs \
                         disk execution_wave={disk_wave} integration_order={disk_order}; the \
                         planner must read execution-plan.yml units[] rather than reconstruct \
                         the schedule by hand (plan 2026-07-30 fix for primary-20260730-002911)"
                    ),
                    ..Default::default()
                });
            }
...
"#;
    ...
    // 2026-07-30 fix: forge.plan.ready payload must mirror the
    // on-disk execution-plan.yml. The previous 2026-07-30 primary
    // run hit a silent dead-lock because the planner hand-built the
    // payload with execution_wave=0 for F1 (and -1 for all other
    // units), the runtime rejected it via validate_wave_schedule
    // Rule 1, and the planner hat never learned why (guidance
    // topic removed). These three tests pin the CLI-side fix.

    fn write_execution_plan(tmp: &TempDir, content: &str) -> std::path::PathBuf {
        let path = tmp.path().join("execution-plan.yml");
        std::fs::write(&path, content).expect("write execution-plan.yml");
        path
    }

    fn aligned_payload(plan_path: &std::path::Path, digest: &str) -> String {
        // Mirror of `tests/fixtures/parallel-forge.yml` style:
        // 9 units, F1 wave=1 order=1 ... U8 wave=6 order=9.
        let plan_path_str = plan_path.display().to_string();
        format!(
            r#"{{
                "plan_path": "docs/plans/x.md",
                "plan_key": "p",
                "development_plan_path": "x.md",
                "execution_plan_path": "{plan_path_str}",
                "execution_plan_digest": "{digest}",
                "wave_total": 6,
                "unit_count": 2,
                "unit_tasks": [
                    {{"task_key": "forge:p:F1", "unit_id": "F1", "title": "F1",
                     "execution_wave": 1, "integration_order": 1,
                     "depends_on_task_keys": []}},
                    {{"task_key": "forge:p:U8", "unit_id": "U8", "title": "U8",
                     "execution_wave": 6, "integration_order": 9,
                     "depends_on_task_keys": ["forge:p:F1"]}}
                ]
            }}"#
        )
    }
    ...
    #[test]
    fn check_forge_plan_ready_disk_consistency_passes_when_payload_matches_disk() {
        let tmp = TempDir::new().unwrap();
...
"#,
        ...
        let payload = aligned_payload(&plan_path, "abc123");
        let result =
            check_forge_plan_ready_disk_consistency("forge.plan.ready", &payload, tmp.path());
        assert!(result.is_ok(), "aligned payload must pass; got {result:?}");
    }

    #[test]
    fn check_forge_plan_ready_disk_consistency_rejects_zero_execution_wave() {
        // Reproduces primary-20260730-002911: planner emitted F1
        // with execution_wave=0 even though disk YAML had it as 1.
        let tmp = TempDir::new().unwrap();
        let plan_path = write_execution_plan(
            &tmp,
            r#"
...
"#,
        ...
        let mut payload: serde_json::Value =
            serde_json::from_str(&aligned_payload(&plan_path, "abc123")).unwrap();
        // Force the historical bug: F1 wave=0.
    ...
    #[test]
    fn check_forge_plan_ready_disk_consistency_rejects_digest_mismatch() {
        let tmp = TempDir::new().unwrap();
        let plan_path = write_execution_plan(
            &tmp,
            r#"
...
"#,
    ...
    #[test]
    fn check_forge_plan_ready_disk_consistency_is_noop_for_other_topics() {
        let tmp = TempDir::new().unwrap();
        // No execution_plan_path in payload; even with a malformed
        // shape, a non-forge.plan.ready topic must short-circuit.
        let result = check_forge_plan_ready_disk_consistency(
