...
// `ColorMode`, `Verbosity`, `OutputFormat`, `ConfigSource`, `HatsSource`,
// the config loader helpers, the emit-allowlist guard, the panic hook, and
// the platform-conditional process-group leadership helper now live in
// `crate::cli::*`. We re-export them here so the rest of `main.rs` and the
// `commands/*` files can keep using the short local names without touching
// every call site.
use crate::cli::{
    ColorMode, ConfigSource, HatsSource, OutputFormat, Verbosity, apply_config_overrides,
    default_config_path, ensure_scratchpad_directory, install_panic_hook,
    load_config_with_overrides, resolve_emit_path, resolve_marker_target,
    resolve_path_from_workspace, resolve_workspace_root, urgent_steer_path_from_workspace,
};
